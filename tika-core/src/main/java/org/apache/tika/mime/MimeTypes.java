 ...
 * <p>
 * The MIME type detection methods that take an {@link InputStream} as an argument will never reads
 * more than {@link #getMinLength()} bytes from the stream. Also the given stream is never
 * {@link InputStream#close() closed}, {@link InputStream#mark(int) marked}, or
 * {@link InputStream#reset() reset} by the methods. Thus a client can use the
 * {@link InputStream#markSupported() mark feature} of the stream (if available) to restore the
 * stream back to the state it was before type detection if it wants to process the stream based on
 * the detected type.
 */
public final class MimeTypes implements Detector, Serializable {
    ...
    public static synchronized MimeTypes getDefaultMimeTypes(ClassLoader classLoader) {
        ...
        if (types == null) {
            try {
                types = MimeTypesFactory.create("tika-mimetypes.xml", "custom-mimetypes.xml",
                                classLoader);
     ...
     * <p>
     * If multiple matches are found, the best (highest priority) matching type is returned. If
     * multiple matches are found with the same priority, then all of these are returned.
     * <p>
     * The given byte array is expected to be at least {@link #getMinLength()} long, or shorter only
     * if the document stream itself is shorter.
     *
     ...
     * <p>
     * The given stream is never {@link InputStream#close() closed}, {@link InputStream#mark(int)
     ...
     *
     * <p>
     * Unlike {@link #forName(String)}, this function will <em>not</em> create a new MimeType and
     * register it. Instead, <code>null</code> will be returned if there is no definition available
     * for the given name.
     *
     * <p>
     * Also, unlike {@link #forName(String)}, this function may return a mime type that has fewer
     ...
     * type does not (e.g. <code>application/xml; charset=UTF-8</code> as a name, compared to just
     * <code>application/xml</code> for the type in the registry), then those parameters will not be
     * included in the returned type.
     *
     * @param name media type name (case-insensitive)
    ...
    /**
     * Adds an alias for the given media type. This method should only be called from
     * {@link MimeType#addAlias(String)}.
     *
     ...
     * being added <b>is</b> or <b>is not</b> a JDK standard regular expression via the
     * <code>isRegex</code> parameter. If the value is set to true, then a JDK standard regex is
     * assumed, otherwise the freedesktop glob type is assumed.
     *
     * @param type media type
     * @param pattern file name pattern
     ...
     * <p>
     * The given stream is expected to support marks, so that this method can reset the stream to
