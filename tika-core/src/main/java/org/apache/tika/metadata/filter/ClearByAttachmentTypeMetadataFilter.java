...
/**
 * This class clears the entire metadata object if the attachment type matches one of the types. The
 * idea is that you might not want to store/transmit metadata for images or specific file types.
 */
    ...
    public void setTypes(List<String> types) throws TikaConfigException {
        for (String t : types) {
            try {
                ...
                int i = 0;
                for (TikaCoreProperties.EmbeddedResourceType type : TikaCoreProperties.EmbeddedResourceType
                                .values()) {
                    if (i++ > 0) {
                        sb.append(", ");
                    }
