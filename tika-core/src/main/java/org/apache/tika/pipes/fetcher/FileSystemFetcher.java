...
public class FileSystemFetcher extends AbstractFetcher implements Initializable {
    ...
    public InputStream fetch(String fetchKey, Metadata metadata)
        ...
        if (!p.toRealPath().startsWith(basePath.toRealPath())) {
            throw new IllegalArgumentException("fetchKey must resolve to be a" +
                    " descendant of the 'basePath'");
        }
    ...
    static boolean isDescendant(Path root, Path descendant) {
        return descendant.toAbsolutePath().normalize().startsWith(
    ...
    public void checkInitialization(InitializableProblemHandler problemHandler) throws TikaConfigException {
        ...
        if (basePath.toAbsolutePath().toString().contains("\u0000")) {
            throw new IllegalArgumentException("base path must not contain \u0000. " +
