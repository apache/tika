...
public class ForkParser implements Parser, Closeable {
     ...
     * <li>Send in a class that extends {@link AbstractRecursiveParserWrapperHandler} and the server
     * will act on the class but not proxy back the data. This can be used, for example, if all you
     * want to do is write to disc, extend {@link AbstractRecursiveParserWrapperHandler} to write to
     * disc when {@link AbstractRecursiveParserWrapperHandler#endDocument(ContentHandler, Metadata)}
     * is called, and the server will take care of the writing via the handler.</li>
     * </ol>
     * </p>
     * <p>
     * <b>NOTE:</b>[0] &quot;the server will proxy back the data as best it can&quot;. If the
     ...
     * endEmbeddedDocument(ContentHandler, Metadata)} or
     * {@link org.apache.tika.sax.RecursiveParserWrapperHandler# endEmbeddedDocument(ContentHandler,
     ...
     * {@link ContentHandler#toString()} on the ContentHandler and set that value with the
     * {@link TikaCoreProperties#TIKA_CONTENT} key and then serialize and proxy that data back.
     * </p>
     *
     ...
     *        parser shuts down a client and creates a new process. If set to -1, the server is
     *        never restarted because of the number of files handled.
     */
