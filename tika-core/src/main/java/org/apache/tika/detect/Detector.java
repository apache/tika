...
public interface Detector extends Serializable {
     ...
     * <code>application/octet-stream</code> if the type of the document can not be detected.
     * <p>
     * If the document input stream is not available, then the first argument may be
     * <code>null</code>. Otherwise the detector may read bytes from the start of the stream to help
     * in type detection. The given stream is guaranteed to support the
     * {@link InputStream#markSupported() mark feature} and the detector is expected to
     * {@link InputStream#mark(int) mark} the stream before reading any bytes from it, and to
     * {@link InputStream#reset() reset} the stream before returning. The stream must not be closed
     * by the detector.
     * <p>
