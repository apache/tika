 ...
 * ({@link #characters(char[], int, int)} or {@link #ignorableWhitespace(char[], int, int)}) passed
 * to the decorated content handler contain only valid XML characters. All invalid characters are
 * replaced with the Unicode replacement character U+FFFD (though a subclass may change this by
 * overriding the {@link #writeReplacement(Output)} method).
 * <p>
 ...
 * </pre>
 * <p>
 * Note that currently this class only detects those invalid characters whose UTF-16 representation
 * fits a single char. Also, this class does not ensure that the UTF-16 encoding of incoming
...
public class SafeContentHandler extends ContentHandlerDecorator {
     ...
     * 
     * // Called only from assert private boolean verifyStartElement(String name) { // TODO: we
     * could strengthen this to do full // XTHML validation, eg you shouldn't start p inside //
     * another p (but ODF parser, at least, seems to // violate this): //if (name.equals("p")) {
     * //assert elements.size() == 0 || !elements.get(elements.size()-1).equals("p"); //}
     * elements.add(name); return true; }
     * 
     * // Called only from assert private boolean verifyEndElement(String name) { assert
     * elements.size() > 0: "end tag=" + name + " with no startElement"; final String currentElement
     * = elements.get(elements.size()-1); assert currentElement.equals(name):
     * "mismatched elements open=" + currentElement + " close=" + name;
     * elements.remove(elements.size()-1); return true; }
     * 
     * // Called only from assert private boolean verifyEndDocument() { assert elements.size() == 0;
     * return true; }
     */
