 ...
 * <p>
 * Every time a document is parsed in Tika, the content is split into SAX events. Those SAX events
 * are handled by a ContentHandler. You can think of these events as marking a tag in an HTML file.
 * Once you're finished parsing, you can call handler.toString(), for example, to get the text
 ...
 * <p>
 * This class is an example of how to combine a ContentHandler and a Metadata. As content is passed
 * to the handler, we first check to see if it matches a textual pattern for a phone number. If the
 * extracted content is a phone number, we add it to the metadata under the key "phonenumbers". So,
 * if you used this ContentHandler when you parsed a document, then called
