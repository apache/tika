...
/**
 * Builds on top of the LuceneIndexer and the Metadata discussions in Chapter 6 to output an RSS (or
 * RDF) feed of files crawled by the LuceneIndexer within the last N minutes.
 */
...
public class RecentFiles {
    ...
    public String generateRSS(Path indexFile) throws CorruptIndexException, IOException {
        ...
        try {
            ...
            GregorianCalendar gc = new java.util.GregorianCalendar(TimeZone.getDefault(),
                            Locale.getDefault());
            gc.setTime(new Date());
    ...
    public String getRSSItem(Document doc) {
        ...
        output.append(emitTag("pubDate",
                        rssDateFormat.format(ISO8601
                                        .parse(doc.get(TikaCoreProperties.CREATED.getName()))),
                        null, null));
