...
public class TikaMimeTypes {
    ...
    public String getMimeTypesHTML() {
        ...
        for (MediaTypeDetails type : types) {
            ...
            if (type.supertype != null) {
                h.append("<div>Super Type: <a href=\"#").append(type.supertype).append("\">").append(type.supertype)
                        .append("</a></div>\n");
            }
            if (type.mime != null) {
    ...
    public String getMimeTypeDetailsHTML(@PathParam("type") String typePart, @PathParam("subtype") String subtype) {
        ...
        if (type.mime != null) {
            ...
            if (!type.mime.getUniformTypeIdentifier().isEmpty()) {
                h.append("<div>Uniform Type Identifier: ").append(type.mime.getUniformTypeIdentifier())
                        .append("</div>\n");
            }
    ...
    @Path("/{type}/{subtype}")
    @Produces(jakarta.ws.rs.core.MediaType.APPLICATION_JSON)
    public String getMimeTypeDetailsJSON(@PathParam("type") String typePart, @PathParam("subtype") String subtype)
            throws IOException {
        MediaTypeDetails type = getMediaType(typePart, subtype);
        Map<String, Object> details = new HashMap<>();
