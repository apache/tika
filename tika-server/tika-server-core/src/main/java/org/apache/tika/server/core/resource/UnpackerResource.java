...
public class UnpackerResource {
    ...
    public static void metadataToCsv(Metadata metadata, OutputStream outputStream)
                    throws IOException {
        try (CSVPrinter writer = new CSVPrinter(new OutputStreamWriter(outputStream, UTF_8),
                        CSVFormat.EXCEL)) {
    ...
    private static class MyEmbeddedDocumentExtractor implements EmbeddedDocumentExtractor {
                ...
                if (bis.hasHitBound()) {
                    throw new IOException(
                                    new TikaMemoryLimitException("An attachment is longer than "
                                                    + "'unpackMaxBytes' (default=100MB, actual="
                                                    + unpackMaxBytes + "). "
         ...
         * { for (Entry entry : sourceDir) { if (entry instanceof DirectoryEntry) { // Need to
         * recurse DirectoryEntry newDir = destDir.createDirectory(entry.getName());
         * copy((DirectoryEntry) entry, newDir); } else { // Copy entry try (InputStream contents =
         * new DocumentInputStream((DocumentEntry) entry)) { destDir.createDocument(entry.getName(),
         * contents); } } } }
         */
    }
}
