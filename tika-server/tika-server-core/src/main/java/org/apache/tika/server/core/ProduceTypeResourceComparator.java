...
/**
 * Resource comparator based to produce type. In an ambiguous call, request handler will be chosen
 * based on the type of data it returns.
 */
public class ProduceTypeResourceComparator implements ResourceComparator {

    /**
     * The prioritized MediaType list. The lower in list (higher index value), the higher priority
     * it has. In case of no matching in this list, it will be treated as media type all.
     */
    ...
    /**
     * Initiates the comparator. Currently, no op.
     */
    public ProduceTypeResourceComparator() {}

    /**
     * Compares the class to handle. Nothing is handled here, and this is handed over to CXF default
     * logic.
     *
    ...
    /**
     * Compares the method to handle. Gets the maximum priority match for both handlers, and chooses
     * the handler that has the maximum priority match.
     *
    ...
    public int compare(OperationResourceInfo oper1, OperationResourceInfo oper2, Message message) {
        ...
        final List<MediaType> acceptTypes = JAXRSUtils
                        .parseMediaTypes((String) message.get(Message.ACCEPT_CONTENT_TYPE));
    ...
    private int compareProduceTypes(OperationResourceInfo oper1, OperationResourceInfo oper2,
        ...
        int oper1Priority = op1Matched.stream().mapToInt(PRIORITIZED_MEDIA_LIST::indexOf).max()
                        .getAsInt();
        int oper2Priority = op2Matched.stream().mapToInt(PRIORITIZED_MEDIA_LIST::indexOf).max()
                        .getAsInt();
