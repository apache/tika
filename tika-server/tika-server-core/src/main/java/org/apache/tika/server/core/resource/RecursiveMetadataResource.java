...
public class RecursiveMetadataResource {
    ...
    public static List<Metadata> parseMetadata(InputStream is, Metadata metadata,
                    ...
                    HandlerConfig handlerConfig) throws Exception {
    ...
    /**
     * Returns an InputStream that can be deserialized as a list of {@link Metadata} objects. The
     * first in the list represents the main document, and the rest represent metadata for the
     * embedded objects. This works recursively through all descendants of the main document, not
     ...
     * <p>
     * Specify the handler for the content (xml, html, text, ignore) in the path:<br/>
    ...
    public Response getMetadataFromMultipart(Attachment att, @Context UriInfo info,
        ...
        return Response.ok(parseMetadataToMetadataList(att.getObject(InputStream.class),
                        new Metadata(), att.getHeaders(), info, buildHandlerConfig(att.getHeaders(),
                                        handlerTypeName, HandlerConfig.PARSE_MODE.RMETA)))
                        .build();
    }
    ...
    public Response getMetadata(InputStream is, @Context HttpHeaders httpHeaders,
                    @Context UriInfo info, @PathParam(HANDLER_TYPE_PARAM) String handlerTypeName)
                    throws Exception {
