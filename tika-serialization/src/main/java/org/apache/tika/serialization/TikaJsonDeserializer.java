...
public class TikaJsonDeserializer {
    ...
    public static Optional deserializeObject(JsonNode root) {
        ...
        if (!root.has(TikaJsonSerializer.INSTANTIATED_CLASS_KEY)) {
            throw new IllegalArgumentException(
                            "need to specify: " + TikaJsonSerializer.INSTANTIATED_CLASS_KEY);
        }
    ...
    private static Map<String, List<Method>> getSetters(Object obj) {
        ...
        for (Method m : obj.getClass().getMethods()) {
            String n = m.getName();
            if (n.startsWith(TikaJsonSerializer.SET) && n.length() > 3
    ...
    private static void tryArray(String name, JsonNode node, Object obj, Method setter)
        ...
        if (argClass.isArray()) {
            ...
            if (elementType == null) {
                throw new IllegalArgumentException(
                                "Can't infer parameterized type for list in: " + node);
            }
            int len = node.size();
    ...
    private static void setNumericValue(String name, JsonNode node, Object obj,
                    List<Method> setters) throws ReflectiveOperationException {
