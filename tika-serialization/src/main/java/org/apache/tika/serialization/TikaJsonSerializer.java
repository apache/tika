...
/**
 * This is a basic serializer that requires that an object: a) have a no-arg constructor b) have
 * both setters and getters for the same parameters with the same names, e.g. setXYZ and getXYZ c)
 * setters and getters have to follow the pattern setX where x is a capital letter d) have maps as
 * parameters where the keys are strings (and the values are strings for now) e) at deserialization
 * time, objects that have setters for enums also have to have a setter for a string value of that
 * enum
 */
public class TikaJsonSerializer {

    public static String INSTANTIATED_CLASS_KEY = "_class";
    static Set<Class> PRIMITIVES = Set.of(int.class, double.class, float.class, long.class,
                    short.class, boolean.class, String.class, byte.class, char.class);
    static Set<Class> BOXED = Set.of(Integer.class, Double.class, Float.class, Long.class,
        ...
        try {
            ...
            for (Map.Entry<String, Method> e : matches.entrySet()) {
                try {
                    ...
                    throw new TikaSerializationException("couldn't write paramName=" + e.getKey(),
                                    ex);
                }
            }
    ...
    private static void serializeCollection(String fieldName, Object obj,
                    JsonGenerator jsonGenerator) throws IOException, TikaSerializationException {
        if (fieldName != null) {
            jsonGenerator.writeFieldName(fieldName);
        }
        Class clazz = obj.getClass();
    ...
    private static void serializePrimitiveAndBoxed(String paramName, Object obj,
