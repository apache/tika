...
public class OpenOfficeContentParser implements Parser {
	...
	/**
	 * Mappings between OpenDocument tag names and XHTML tag names (including attributes).
	...
	static {
		...
		MAPPINGS.put(new QName(TEXT_NS,"a"),new TargetElement(XHTML,"a",
			Collections.singletonMap(new QName(XLINK_NS,"href"), new QName("href"))
		));
	...
	public void parse(InputStream stream, ContentHandler handler, Metadata metadata)
		...
		final DefaultHandler dh = new ElementMappingContentHandler(xhtml, MAPPINGS) {
			...
			// helper for checking tags which need complete filtering (with sub-tags)
			private final boolean needsCompleteFiltering(String namespaceURI, String localName) {
				return (
					(TEXT_NS.equals(namespaceURI) && (localName.endsWith("-template") || localName.endsWith("-style"))) ||
					(TABLE_NS.equals(namespaceURI) && "covered-table-cell".equals(localName))
				);
			}
			...
			public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
				// keep track of current node type. If it is a text node, a bit at the current depth ist set in textNodeStack.
				// characters() checks the top bit to determine, if the actual node is a text node to print out
				// nodeDepth contains the depth of the current node and also marks top of stack.
				assert nodeDepth>=0;
				textNodeStack.set(nodeDepth++, TEXT_NS.equals(namespaceURI));
				// filter *all* content of some tags
				assert completelyFiltered>=0;
				if (needsCompleteFiltering(namespaceURI,localName)) completelyFiltered++;
				// call next handler if no filtering
				if (completelyFiltered==0) {
					// special handling of text:h, that are directly passed to xhtml handler
		...
		try {
			...
			parser.parse(new CloseShieldInputStream(stream),new NSNormalizerContentHandler(dh));
