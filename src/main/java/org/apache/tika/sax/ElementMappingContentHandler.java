...
/**
 * Content handler decorator that maps element <code>QName</code>s using a <code>Map</code>.
 * Not mappable elements are not forwarded.
 * Attributes may also be mapped (for each element different using a <code>Map</code> for attributes),
 * not mappable attributes are not forwarded.
 * The default is to not map any attributes and therefore do not forward any of them.
 */
