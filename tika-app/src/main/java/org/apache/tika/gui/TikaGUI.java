...
public class TikaGUI extends JFrame implements ActionListener, HyperlinkListener {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SwingUtilities.invokeLater(() -> new TikaGUI(
                new DigestingParser(new AutoDetectParser(finalConfig), new CommonsDigester(MAX_MARK,
                        CommonsDigester.DigestAlgorithm.MD5, CommonsDigester.DigestAlgorithm.SHA256), false),
                finalConfig).setVisible(true));
    }
    ...
    private void handleStream(InputStream input, Metadata md) throws Exception {
        ...
        ContentHandler handler = new TeeContentHandler(getHtmlHandler(htmlBuffer), getTextContentHandler(textBuffer),
        ...
        if (isReset) {
            RecursiveParserWrapperHandler recursiveParserWrapperHandler = new RecursiveParserWrapperHandler(
                    new BasicContentHandlerFactory(BasicContentHandlerFactory.HANDLER_TYPE.BODY, -1), -1);
