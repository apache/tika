 ...
 * nothing else. A candidate is a raster image that is a {@code THUMBNAIL} at depth 1 (stored
 * thumbnail), a rendered {@code THUMBNAIL} at depth 2 (the rasterized vector thumbnail of an
...
public class ThumbnailUnpackSelector implements UnpackSelector {
    ...
    /** The stateless rule; {@code /rmeta} clients pick the first embedded document it accepts. */
    public static boolean isCandidate(Metadata metadata) {
        ...
        if (TikaCoreProperties.EmbeddedResourceType.THUMBNAIL.name().equals(type)) {
            return depth == 1
                    || (depth == 2 && metadata.get(Rendering.RENDERED_BY) != null);
        }
