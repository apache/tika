...
class PipesRequestDeserializer extends JsonDeserializer<PipesRequest> {
    ...
    public PipesRequest deserialize(JsonParser p, DeserializationContext ctxt)
        ...
        if (tupleNode == null) {
            throw new IOException("PipesRequest is missing its '" + PipesRequest.TUPLE + "' field");
        }
        FetchEmitTuple tuple = p.getCodec().treeToValue(tupleNode, FetchEmitTuple.class);
        JsonNode bytesNode = root.get(PipesRequest.INLINE_BYTES);
        byte[] inlineBytes = bytesNode == null ? null : bytesNode.binaryValue();
