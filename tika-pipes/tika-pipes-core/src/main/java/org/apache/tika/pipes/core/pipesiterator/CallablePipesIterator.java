...
public class CallablePipesIterator implements Callable<Long> {
     ...
     * fetchemittuples to the queue. This sets the number of
     * {@link PipesIterator#COMPLETED_SEMAPHORE} to 1. This means that your consumers must put the
     ...
     *        If the consumers are adding this back to the queue when they find it, then this should
     *        be set to 1, otherwise, for a single semaphore for each consumer, set this to the
     *        number of consumers
     */
    public CallablePipesIterator(PipesIterator pipesIterator,
                    ArrayBlockingQueue<FetchEmitTuple> queue, long timeoutMillis,
