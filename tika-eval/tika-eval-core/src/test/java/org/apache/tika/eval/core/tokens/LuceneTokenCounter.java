...
/**
 * Experimental class uses Lucene's MemoryIndex to effectively build the token info.
 */
public class LuceneTokenCounter {
    ...
    void count(String field) throws IOException {
        ...
        fieldStats.put(field, new TokenStatistics(uniqueTokenCount, tokenCountInt, queue.getArray(),
                        ent, summStats));
    }
    ...
    /*
     * public ContrastStatistics contrast(String fieldA, String fieldB) throws IOException { long
     * diceDenom = getUniqueTokenCount(fieldA) + getUniqueTokenCount(fieldB);
     * 
     * long diceNum = 0; long overlapNum = 0;
     * 
     * Terms termsA = getTerms(fieldA); Terms termsB = getTerms(fieldB);
     * 
     * TermsEnum termsEnumA = termsA.iterator(); TermsEnum termsEnumB = termsB.iterator();
     * 
     * BytesRef bytesRefA = termsEnumA.next(); BytesRef bytesRefB = termsEnumB.next();
     * 
     * while (bytesRefA != null) { int compare = bytesRefA.compareTo(bytesRefB); while (compare > 0)
     * { if (bytesRefB == null) { break; } //handle term in B, but not A
     * 
     * compare = bytesRefA.compareTo(bytesRefB); bytesRefB = termsEnumB.next(); } if (compare == 0)
     * { diceNum += 2; overlapNum += 2 * Math.min(termsEnumA.totalTermFreq(),
     * termsEnumB.totalTermFreq()); }
     * 
     * bytesRefA = termsEnumA.next(); }
     * 
     * 
     * for (PairCount p : tokens.values()) { if (p.a > 0 && p.b > 0) { diceNum += 2; overlapNum += 2
     * * Math.min(p.a, p.b); } }
     * 
     * float dice = (float) diceNum / (float) diceDenom; float overlap = (float) overlapNum /
     * (float) (theseTokens.getTokenCount() + thoseTokens.getTokenCount()); }
     */
}
