...
public class ByteNgramFeatureExtractor implements FeatureExtractor<byte[]> {
     ...
     * bucket counts (e.g. 65536) in tight training loops.
     *
     ...
     * }</pre>
     * This keeps the dense buffer zeroed for the next call without a full
     * {@code Arrays.fill}.
     *
     ...
     * @param touched caller-allocated buffer; receives the indices of non-zero
     *                buckets (length {@code numBuckets} is a safe upper bound,
     *                but 2 * probe.length + 2 is the true worst case)
