...
public class EnviHeaderParserTest {
    ...
    public void testParseGlobalMetadata() throws Exception {
        ...
        assertContains("<p>map info = {Sinusoidal, 1.5000, 1.5000, -10007091.3643, "
                        + "5559289.2856, 4.6331271653e+02, 4.6331271653e+02, , units=Meters}</p>",
                        content);
        assertContains("content=\"application/envi.hdr\"", content);
        assertContains("projection info = {16, 6371007.2, 0.000000, 0.0, 0.0, Sinusoidal, units=Meters}",
                        content);
    }
    ...
    public void testParseGlobalMetadataMultiLineMetadataValues() throws Exception {
        ...
        IOUtils.copy(EnviHeaderParserTest.class
                        .getResourceAsStream("/ground-truth" + "/EnviHeaderGroundTruth.txt"), bos);
        ...
        String content = handler.toString();
        assertContains("<body><p>ENVI</p>", content);
        assertContains("<p>description = {  Georeferenced Image built from input GLT. "
                        + "[Wed Jun 10 04:37:54 2015] [Wed  Jun 10 04:48:52 2015]}</p>", content);
        assertContains("<p>samples = 739</p>", content);
        assertContains("<p>lat/lon = { 36.79077627261556, -108.48370867914815 }</p>", content);
        assertContains("<p>map info = { UTM , 1.000 , 1.000 , 724522.127 , 4074620.759 , "
                        + "1.1000000000e+00 , 1.1000000000e+00 , 12 , North , "
                        + "WGS-84 , units=Meters , " + "rotation=75.00000000 }</p>", content);
