 ...
 * <p>
 * Tika extractors decorate POI extractors so that the parsed content of documents is returned as a
 * sequence of XHTML SAX events. Subclasses must implement the buildXHTML method
 * {@link #buildXHTML(XHTMLContentHandler)} that populates the {@link XHTMLContentHandler} object
...
public abstract class AbstractOOXMLExtractor implements OOXMLExtractor {
                ...
                try (InputStream tStream = tPart.getInputStream()) {
                    ...
                    if (embeddedExtractor.shouldParseEmbedded(thumbnailMetadata)) {
                        embeddedExtractor.parseEmbedded(TikaInputStream.get(tStream),
                                        new EmbeddedContentHandler(handler), thumbnailMetadata,
                                        false);
                    }
                }
            }
        ...
        try {
            for (PackagePart source : getMainDocumentParts()) {
                ...
                for (PackageRelationship rel : source.getRelationships()) {
                    try {
                        handleEmbeddedPart(source, rel, xhtml, metadata, embeddedPartMetadataMap,
                                        handledTarget);
    ...
    private void handleEmbeddedOLE(PackagePart part, XHTMLContentHandler xhtml, String rel,
                    Metadata parentMetadata, EmbeddedPartMetadata embeddedPartMetadata)
    ...
    private String getPackageEntryName(DirectoryNode root) {
        ...
        /*
         * raw CorelDraw stream may be in an ole bundle but there can be other resources for the
         * image in other streams under root...think about this... see:
         * AZG2X4VXB3KIEDT3OVZC4R645KU5VSOF if (root.hasEntry("CorelDRAW")) { return "CorelDRAW"; }
         */
        return null;
    }
    ...
    protected Map<String, String> loadLinkedRelationships(PackagePart bodyPart,
                    boolean includeInternal, Metadata metadata) {
        ...
        try {
            ...
            for (String rel : EMBEDDED_RELATIONSHIPS) {
                ...
                for (int i = 0; i < prc.size(); i++) {
                    ...
                    String id = pr.getId();
                    String uriString = (pr.getTargetURI() == null) ? null
                                    : pr.getTargetURI().toString();
    ...
    void handleGeneralTextContainingPart(String contentType, String xhtmlClassLabel,
                    PackagePart parentPart, Metadata parentMetadata, ContentHandler contentHandler)
                    throws SAXException {
            ...
            for (int i = 0; i < relatedPartPRC.size(); i++) {
                ...
                try {
                    ...
                    try (InputStream stream = relatedPartPart.getInputStream()) {
                        XMLReaderUtils.parseSAX(stream, new EmbeddedContentHandler(contentHandler),
                                        context);
