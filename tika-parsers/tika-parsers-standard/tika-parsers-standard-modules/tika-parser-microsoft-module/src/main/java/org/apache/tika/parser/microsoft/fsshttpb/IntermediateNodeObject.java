...
public class IntermediateNodeObject extends NodeObject
    {
        ...
        @Override public List<Byte> GetContent()
        {
        ...
        public static class RootNodeObjectBuilder
        {
            ...
            public IntermediateNodeObject BuildFromListOfDataElements(List<DataElement> dataElements, ExGuid storageIndexExGuid)
            {
                    ...
                    && DataElementUtils.TryAnalyzeWhetherConfirmSchema(dataElements, storageIndexExGuid))
                {
                    ...
                    List<ObjectGroupDataElementData> objectGroupList = DataElementUtils.GetDataObjectDataElementData(dataElements, storageIndexExGuid, rootObjectExGUID);
...
//                        }
                    ...
                    {
                        throw new InvalidOperationException(String.format("There is no root extended GUID value %s", DataElementUtils.RootExGuid.toString()));
                    }
                }
            ...
            private IntermediateNodeObject BuildFromListOfObjectGroupDataElementData(List<ObjectGroupDataElementData> objectGroupList, ExGuid rootExGuid)
            {
...
//                }
                ...
                if (StreamObject.TryGetCurrent(ByteUtil.toByteArray(root.Data.Content), index, rootNode, IntermediateNodeObject.class))
                {
                    ...
                    for (ExGuid extGuid : root.ObjectExGUIDArray.Content)
                    {
                        ...
                        rootNode.get().IntermediateNodeObjectList.add(new LeafNodeObject.IntermediateNodeObjectBuilder().Build(objectGroupList, intermediateData, extGuid));
...
//                    }
                ...
                else
                {
                    ...
                    rootNode.get().IntermediateNodeObjectList.add(new LeafNodeObject.IntermediateNodeObjectBuilder().Build(objectGroupList, root, rootExGuid));
                    rootNode.get().DataSize = new DataSizeObject();
                    rootNode.get().DataSize.DataSize = (long)rootNode.get().IntermediateNodeObjectList.stream().mapToDouble(o -> (double)o.DataSize.DataSize).sum();
                }
...
//                }
            ...
            private ObjectGroupObjectData FindByExGuid(List<ObjectGroupDataElementData> objectGroupList, ExGuid extendedGuid, AtomicReference<ObjectGroupObjectDeclare> declare)
            {
