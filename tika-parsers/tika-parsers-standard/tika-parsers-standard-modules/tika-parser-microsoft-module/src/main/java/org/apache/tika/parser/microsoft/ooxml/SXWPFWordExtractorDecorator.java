...
public class SXWPFWordExtractorDecorator extends AbstractOOXMLExtractor {
    ...
    private final static String[] MAIN_STORY_PART_RELATIONS = new String[]{XWPFRelation.DOCUMENT.getContentType(),
            XWPFRelation.MACRO_DOCUMENT.getContentType(), XWPFRelation.TEMPLATE.getContentType(),
    ...
    private void handleDocumentPart(PackagePart documentPart, XHTMLContentHandler xhtml)
        ...
        for (String rel : new String[]{AbstractOOXMLExtractor.RELATION_DIAGRAM_DATA, XSSFRelation.CHART.getRelation(),
                XWPFRelation.FOOTNOTE.getRelation(), XWPFRelation.COMMENT.getRelation(),
                XWPFRelation.FOOTER.getRelation(), XWPFRelation.ENDNOTE.getRelation(),}) {
            //skip footers if we shouldn't extract them
            if (!config.isIncludeHeadersAndFooters() && rel.equals(XWPFRelation.FOOTER.getRelation())) {
                continue;
            }
            try {
        ...
        try (InputStream stream = packagePart.getInputStream()) {
            XMLReaderUtils.parseSAX(CloseShieldInputStream.wrap(stream),
                    new EmbeddedContentHandler(new OOXMLWordAndPowerPointTextHandler(
                            ...
                            config.isIncludeShapeBasedContent(), config.isConcatenatePhoneticRuns())),
                    context);
