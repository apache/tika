 ...
 * specified in [MS-ONE] section 2.1.12.
 * <p>
 ...
 * PropertySet.rgPrids array until every property has been read.
 * <p>
 * The number of bytes read for each property is specified by the PropertyID.type field.
 * <p>
 * The data for a property that is a reference to one or more objects (section 2.1.5) is contained
 * in the streams within an ObjectSpaceObjectPropSet structure (OIDs.body, OSIDs.body,
 * ContextIDs.body).
 * <p>
 ...
 * bytes) is read from the corresponding stream in the ObjectSpaceObjectPropSet structure.
 * <p>
 * If the PropertyID.type field specifies an array of objects (0x9, 0xB, 0xD), an unsigned integer
 * (4 bytes) is read from the PropertySet.rgDatastream and specifies the number of CompactID
 * structures (section 2.2.2) to read from the corresponding stream in the ObjectSpaceObjectPropSet
 * structure.
 * <p>
 * The streams for each PropertyID.type field are given by the following table.
 * <p>
 * 0x8 (ObjectID, section 2.6.6) - ObjectSpaceObjectPropSet.OIDs.body 0x9 (ArrayOfObjectIDs, section
 * 2.6.6) - ObjectSpaceObjectPropSet.OIDs.body 0xA (ObjectSpaceID, section 2.6.6) -
 * ObjectSpaceObjectPropSet.OSIDs.body 0xB (ArrayOfObjectSpaceIDs, section 2.6.6) -
 * ObjectSpaceObjectPropSet.OSIDs.body 0xC (ContextID, section 2.6.6) -
 * ObjectSpaceObjectPropSet.ContextIDs.body 0xD (ArrayOfContextIDs, section 2.6.6) -
