...
public abstract class StreamObject implements IFSSHTTPBSerializable {
    ...
    private static final Set<StreamObjectTypeHeaderStart> compoundTypes = new HashSet<>(Arrays.asList(
            StreamObjectTypeHeaderStart.DataElement, StreamObjectTypeHeaderStart.Knowledge,
            StreamObjectTypeHeaderStart.CellKnowledge, StreamObjectTypeHeaderStart.DataElementPackage,
            StreamObjectTypeHeaderStart.ObjectGroupDeclarations, StreamObjectTypeHeaderStart.ObjectGroupData,
            StreamObjectTypeHeaderStart.WaterlineKnowledge, StreamObjectTypeHeaderStart.ContentTagKnowledge,
            StreamObjectTypeHeaderStart.Request, StreamObjectTypeHeaderStart.FsshttpbSubResponse,
            StreamObjectTypeHeaderStart.SubRequest, StreamObjectTypeHeaderStart.ReadAccessResponse,
            StreamObjectTypeHeaderStart.SpecializedKnowledge, StreamObjectTypeHeaderStart.WriteAccessResponse,
            StreamObjectTypeHeaderStart.QueryChangesFilter, StreamObjectTypeHeaderStart.ResponseError,
            StreamObjectTypeHeaderStart.UserAgent, StreamObjectTypeHeaderStart.FragmentKnowledge,
            StreamObjectTypeHeaderStart.ObjectGroupMetadataDeclarations, StreamObjectTypeHeaderStart.LeafNodeObject,
            StreamObjectTypeHeaderStart.IntermediateNodeObject, StreamObjectTypeHeaderStart.TargetPartitionId));
    ...
    public static <T extends StreamObject> T getCurrent(byte[] byteArray, AtomicInteger index, Class<T> clazz)
        ...
        if (!streamObject.getClass().equals(clazz)) {
            ...
            throw new StreamObjectParseErrorException(tmpIndex.get(), clazz.getName(), String.format(Locale.US,
                    "Failed to get stream object as expect type %s, actual type is %s", clazz.getName(), destClassName),
                    null);
        }
     ...
     */
    public static StreamObject parseStreamObject(StreamObjectHeaderStart header, byte[] byteArray, AtomicInteger index)
        ...
        if (streamObjectTypeMapping.containsKey(header.type)) {
            ...
            try {
                streamObject = (StreamObject) headerTypeClass.getDeclaredConstructor().newInstance();
        ...
        tmpIndex -= header.headerType == StreamObjectHeaderStart.STREAM_OBJECT_HEADER_START_16_BIT ? 2 : 4;
        throw new StreamObjectParseErrorException(tmpIndex, "Unknown",
    ...
    public static <T extends StreamObject> boolean tryGetCurrent(byte[] byteArray, AtomicInteger index,
            AtomicReference<T> streamObject, Class<T> clazz) throws TikaException, IOException {
        AtomicInteger tmpIndex = new AtomicInteger(index.get());
