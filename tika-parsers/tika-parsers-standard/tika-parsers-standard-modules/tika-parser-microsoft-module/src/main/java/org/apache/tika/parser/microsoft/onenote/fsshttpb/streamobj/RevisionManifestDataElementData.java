    ...
    public int deserializeDataElementDataFromByteArray(byte[] byteArray, int startIndex)
        ...
        while ((headerLength =
            ...
            if (header.get().type == StreamObjectTypeHeaderStart.RevisionManifestRootDeclare) {
                ...
                this.revisionManifestRootDeclareList.add((RevisionManifestRootDeclare) StreamObject
                                .parseStreamObject(header.get(), byteArray, index));
            } else if (header
                            .get().type == StreamObjectTypeHeaderStart.RevisionManifestObjectGroupReferences) {
                index.addAndGet(headerLength);
                this.revisionManifestObjectGroupReferences
                                .add((RevisionManifestObjectGroupReferences) StreamObject
                                                .parseStreamObject(header.get(), byteArray, index));
            } else {
