...
public class OneNoteParser implements Parser {
    ...
    public void parse(InputStream stream, ContentHandler handler, Metadata metadata,
        ...
        try (TemporaryResources temporaryResources = new TemporaryResources();
            ...
            if (header.isMsOneStoreFormat()) {
                ...
                metadata.set(ONE_NOTE_PREFIX + "buildNumberLastWroteToFile", "0x" + Long
                                .toHexString(oneNoteDocument.header.buildNumberLastWroteToFile));
                metadata.set(ONE_NOTE_PREFIX + "buildNumberNewestWritten", "0x" + Long
                                .toHexString(oneNoteDocument.header.buildNumberNewestWritten));
                metadata.set(ONE_NOTE_PREFIX + "buildNumberOldestWritten", "0x" + Long
                                .toHexString(oneNoteDocument.header.buildNumberOldestWritten));
                metadata.set(ONE_NOTE_PREFIX + "cbExpectedFileLength", "0x"
                                + Long.toHexString(oneNoteDocument.header.cbExpectedFileLength));
                metadata.set(ONE_NOTE_PREFIX + "cbFreeSpaceInFreeChunkList", "0x" + Long
                                .toHexString(oneNoteDocument.header.cbFreeSpaceInFreeChunkList));
                metadata.set(ONE_NOTE_PREFIX + "cbLegacyExpectedFileLength", "0x" + Long
                                .toHexString(oneNoteDocument.header.cbLegacyExpectedFileLength));
                metadata.set(ONE_NOTE_PREFIX + "cbLegacyFreeSpaceInFreeChunkList", "0x" + Long
                                .toHexString(oneNoteDocument.header.cbLegacyFreeSpaceInFreeChunkList));
                metadata.set(ONE_NOTE_PREFIX + "crcName",
                                "0x" + Long.toHexString(oneNoteDocument.header.crcName));
                metadata.set(ONE_NOTE_PREFIX + "cTransactionsInLog",
                                "0x" + Long.toHexString(oneNoteDocument.header.cTransactionsInLog));
                metadata.set(ONE_NOTE_PREFIX + "ffvLastCodeThatWroteToThisFile", "0x" + Long
                                .toHexString(oneNoteDocument.header.ffvLastCodeThatWroteToThisFile));
                metadata.set(ONE_NOTE_PREFIX + "ffvNewestCodeThatHasWrittenToThisFile", "0x" + Long
                                .toHexString(oneNoteDocument.header.ffvNewestCodeThatHasWrittenToThisFile));
                metadata.set(ONE_NOTE_PREFIX + "ffvOldestCodeThatMayReadThisFile", "0x" + Long
                                .toHexString(oneNoteDocument.header.ffvOldestCodeThatMayReadThisFile));
                metadata.set(ONE_NOTE_PREFIX + "ffvOldestCodeThatHasWrittenToThisFile", "0x" + Long
                                .toHexString(oneNoteDocument.header.ffvOldestCodeThatHasWrittenToThisFile));
                metadata.set(ONE_NOTE_PREFIX + "grfDebugLogFlags",
                                "0x" + Long.toHexString(oneNoteDocument.header.grfDebugLogFlags));
                metadata.set(ONE_NOTE_PREFIX + "nFileVersionGeneration", "0x"
                ...
                OneNoteTreeWalker oneNoteTreeWalker = new OneNoteTreeWalker(options,
                                oneNoteDocument, oneNoteDirectFileResource, xhtml, metadata,
                                context, roleAndContext);
                ...
                try {
                    ...
                    MSOneStorePackage pkg = onenoteParser
                                    .parse(alternatePackageOneStoreFile.dataElementPackage);
     ...
     * <p>
     * This won't actually have the binary data of the sections, but it's more of a metadata
     * structure that contains the general structure of the container and contains offset positions
     * of where to find the binary data we care about.
     * <p>
     * OneNote files are of format:
     * <p>
     * The header (section 2.3.1 in MS-ONESTORE) is the first 1024 bytes of the file. It contains
     * references to the other structures in the file as well as metadata about the file. The free
     * chunk list (section 2.3.2 in MS-ONESTORE) defines where there are free spaces in the file
     * where data can be written. The transaction log (section 2.3.3 in MS-ONESTORE) stores the
     * state and length of each file node list (section 2.4 in MS-ONESTORE) in the file. The hashed
     * chunk list (section 2.3.4 in MS-ONESTORE) stores read-only objects in the file that can be
     * referenced by multiple revisions (section 2.1.8 in MS-ONESTORE). The root file node list
     * (section 2.1.14 in MS-ONESTORE) is the file node list that is the root of the tree of all
     * file node lists in the file.
     * <p>
     ...
     *         rather it just contains the data pointers and metadata.
