...
public class RevisionManifestDataElementData extends DataElementData
    {
        ...
        @Override public int DeserializeDataElementDataFromByteArray(byte[] byteArray, int startIndex)
        {
            ...
            while ((headerLength = StreamObjectHeaderStart.TryParse(byteArray, index.get(), header)) != 0)
            {
                ...
                {
                    index.addAndGet(headerLength);
                    this.RevisionManifestObjectGroupReferencesList.add((RevisionManifestObjectGroupReferences)StreamObject.ParseStreamObject(header.get(), byteArray, index));
                }
                else
                {
                    throw new DataElementParseErrorException(index.get(), "Failed to parse RevisionManifestDataElement, expect the inner object type RevisionManifestRootDeclare or RevisionManifestObjectGroupReferences, but actual type value is " + header.get().type, null);
                }
            }
        ...
        @Override public List<Byte> SerializeToByteList()
        {
