...
/// <summary>
    ...
    public class StreamObjectHeaderStart32bit extends StreamObjectHeaderStart
    {
        ...
        protected int DoDeserializeFromByteArray(byte[] byteArray, int startIndex)
        {
            ...
            {
                throw new RuntimeException(String.format("Failed to get the StreamObjectHeaderStart32bit header type value, expect value %s, but actual value is %s", StreamObjectHeaderStart.StreamObjectHeaderStart32bit, this.headerType));
            }
            ...
            if (type == null)
            {
                throw new RuntimeException(String.format("Failed to get the StreamObjectHeaderStart32bit type value, the value %s is not defined", typeValue));
            }

            if (StreamObject.getCompoundTypes().contains(this.type) && this.compound != 1)
            {
                throw new RuntimeException(String.format("Failed to parse the StreamObjectHeaderStart32bit header. If the type value is %s then the compound value should 1, but actual value is 0", typeValue));
            }
