...
public class StorageIndexDataElementData extends DataElementData {
    ...
    public int deserializeDataElementDataFromByteArray(byte[] byteArray, int startIndex)
        ...
        while ((headerLength =
                        StreamObjectHeaderStart.tryParse(byteArray, index.get(), header)) != 0) {
            ...
            if (header.get().type == StreamObjectTypeHeaderStart.StorageIndexManifestMapping) {
                ...
                this.storageIndexCellMappingList.add((StorageIndexCellMapping) StreamObject
                                .parseStreamObject(header.get(), byteArray, index));
            } else if (header
                            .get().type == StreamObjectTypeHeaderStart.StorageIndexRevisionMapping) {
                this.storageIndexRevisionMappingList.add((StorageIndexRevisionMapping) StreamObject
                                .parseStreamObject(header.get(), byteArray, index));
            } else {
                throw new DataElementParseErrorException(index.get() - headerLength,
