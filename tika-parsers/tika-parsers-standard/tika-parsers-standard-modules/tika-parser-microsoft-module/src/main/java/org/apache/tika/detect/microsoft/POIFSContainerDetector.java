...
/**
 * A detector that works on a POIFS OLE2 document to figure out exactly what the file is. This
 * should work for all OLE2 documents, whether they are ones supported by POI or not.
 */
public class POIFSContainerDetector implements Detector {
    ...
    private static final String SW_DOC_MGR_TEMP_STORAGE =
                    "SwDocMgrTempStorage".toUpperCase(Locale.ROOT);

    private static final String STAR_CALC_DOCUMENT = "StarCalcDocument".toUpperCase(Locale.ROOT);

    private static final String STAR_WRITER_DOCUMENT =
                    "StarWriterDocument".toUpperCase(Locale.ROOT);
    ...
    private static final String DRM_ENCRYPTED_DATA_SPACE =
                    "DRMEncryptedDataSpace".toUpperCase(Locale.ROOT);
    ...
    private static final String POWERPOINT_DOCUMENT =
                    "PowerPoint Document".toUpperCase(Locale.ROOT);
    ...
    /**
     * An ASCII String "MSGraph.Chart" for embedded MSGraph files The full designator includes a
     * version, e.g. MSGraph.Chart.8
     */
     ...
     * streams within the file. In some cases the detection may need access to the root
     * {@link DirectoryEntry} of that file for best results. The entry can be given as a second,
     * optional argument.
     *
     * <p/>
     * Following
     *
     * <a href=
     * "https://learn.microsoft.com/en-us/openspecs/windows_protocols/ms-cfb/60fe8611-66c3-496b-b70d-a504c94c9ace">2.6.1
     * of MS-CFB </a>, The detection is performed on case insensitive entry names.
     *
     * @param anyCaseNames
     * @param root
     * @return
    ...
    public static MediaType detect(Set<String> anyCaseNames, DirectoryEntry root) {
        ...
        if (ucNames.contains(SW_DOC_CONTENT_MGR) && ucNames.contains(SW_DOC_MGR_TEMP_STORAGE)) {
            ...
            if (root == null) {
                /*
                 * This is either StarOfficeDraw or StarOfficeImpress, we have to consult the
                 * CompObj to distinguish them, if this method is called in "legacy mode", without
                 * the root, just return x-tika-msoffice. The one-argument method is only for
                 * backward compatibility, if someone calls old API he/she can get the old result.
                 */
                return OLE;
            } else {
            ...
            return OLE;
        } else if (ucNames.contains(COMP_OBJ_STRING) && (ucNames.contains(PROPS)
                        || ucNames.contains(PROPS_9) || ucNames.contains(PROPS_12))) {
    ...
    private static MediaType processCompObjFormatType(DirectoryEntry root) {
        try {
            ...
            if (entry != null && entry.isDocumentEntry()) {
                ...
                /*
                 * This array contains a string with a normal ASCII name of the application used to
                 * create this file. We want to search for that name.
                 */
            ...
            /*
             * "root.getEntry" can throw FileNotFoundException. The code inside "if" can throw
             * IOExceptions. Theoretically. Practically no exceptions will likely ever appear.
             *
             * Swallow all of them. If any occur, we just assume that we can't distinguish between
             * Draw and Impress and return something safe: x-tika-msoffice
             */
        }
        return OLE;
    }
    ...
    /**
     * If a TikaInputStream is passed in to {@link #detect(InputStream, Metadata)}, and there is not
     * an underlying file, this detector will spool up to {@link #markLimit} to disk. If the stream
     * was read in entirety (e.g. the spooled file is not truncated), this detector will open the
     * file with POI and perform detection. If the spooled file is truncated, the detector will
     * return {@link #OLE} (or {@link MediaType#OCTET_STREAM} if there's no OLE header).
     * <p>
     * As of Tika 1.21, this detector respects the legacy behavior of not performing detection on a
     * non-TikaInputStream.
     *
     * @param markLimit
     */
    ...
    private boolean isOleHeader(InputStream input) throws IOException {
        input.mark(8);
        try {
            return (input.read() == 0xd0 && input.read() == 0xcf && input.read() == 0x11
    ...
    public static Set<String> tryOpenContainerOnTikaInputStream(InputStream input,
                    Metadata metadata) {
