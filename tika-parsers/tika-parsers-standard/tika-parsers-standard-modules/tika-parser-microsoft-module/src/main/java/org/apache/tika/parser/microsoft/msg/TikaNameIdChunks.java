...
public final class TikaNameIdChunks implements ChunkGroup {
    ...
    /**
     * Holds all the chunks that were found, keyed by id. Not clear if we need a list or if we can
     * rely on a unique id
     */
    ...
    public void record(Chunk chunk) {
        ...
        List<Chunk> chunkList =
                        chunksById.computeIfAbsent(chunk.getChunkId(), k -> new ArrayList<>());
        chunkList.add(chunk);
    }
    ...
    private void loadTags() {
        ...
        if (guidStream == null || entryStream == null || stringStream == null
                        || entryStreamBytes == null) {
            return;
        }
        LittleEndianByteArrayInputStream leis =
                        new LittleEndianByteArrayInputStream(entryStreamBytes);
        for (int i = 0; i < entryStreamBytes.length / 8; i++) {
            ...
            if (tag > 0 && tag < Integer.MAX_VALUE) {
                List<MAPITag> tagList =
                                mapiTagMap.computeIfAbsent((int) tag, k -> new ArrayList<>());
    ...
    /**
     * Get property tag id by property set GUID and string name or numerical name from named
     * properties mapping
     *
     * @param guid Property set GUID in registry format without brackets. May be one of the PS_* or
     *        PSETID_* constants
     * @param name Property name in case of string named property
     * @param id Property id in case of numerical named property
     * @return Property tag which can be matched with {@link MAPIProperty#id} or 0 if the property
     *         could not be found.
     */
    ...
    private long getPropertyTag(long streamID, long nameOffset, long propertyNameCRC32) {
        ...
        for (Chunk chunk : chunks) {
            ...
            LittleEndianByteArrayInputStream leis =
                            new LittleEndianByteArrayInputStream(matchChunkBytes);
    ...
    private static boolean matchesProperty(int propertyKind, long nameOffset, String name,
                    String propertyName, long id) {
        return
    ...
    private long getStreamID(int propertyKind, int nameOffset, ClassID guid, int guidIndex,
        ...
        if (stringBytes.length > nameOffset) {
            ...
            if (stringBytes.length >= nameOffset + 4 + nameLength) {
                ...
                String propertyName = new String(stringBytes, nameStart, (int) nameLength,
                                StringUtil.UTF16LE);
    ...
    /**
     * Calculates the CRC32 of the given bytes (conforms to RFC 1510, SSH-1). The CRC32 calculation
     * is similar to the standard one as demonstrated in RFC 1952, but with the inversion (before
     * and after the calculation) omitted.
     * <ul>
