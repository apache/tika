...
/**
 * Extracts files embedded in EMF and offers a very rough capability to extract text if there is
 * text stored in the EMF.
 * <p/>
 * To improve text extraction, we'd have to implement quite a bit more at the POI level. We'd want
 * to track changes in font and use that information for identifying character sets, inserting
 * spaces and new lines.
 * <p/>
 * We're also relying on storage order for text order, which isn't great. We'd have to do something
 * like what PDFBox or XPS do to sort the runs and then put the cow back together from the
 * hamburger...lol...
 */
public class EMFParser implements Parser {
    ...
    private static void handleEmbedded(byte[] data,
                    EmbeddedDocumentExtractor embeddedDocumentExtractor, ContentHandler handler)
                    throws TikaException, SAXException {
        try (TikaInputStream tis = TikaInputStream.get(data)) {
            Metadata embeddedMetadata = new Metadata();
            if (embeddedDocumentExtractor.shouldParseEmbedded(embeddedMetadata)) {
                embeddedDocumentExtractor.parseEmbedded(tis, new EmbeddedContentHandler(handler),
    ...
    public void parse(InputStream stream, ContentHandler handler, Metadata metadata,
        ...
        try {
            ...
            // NOTE that we're just scraping the text out in storage order. The proper way to do
            // this
            ...
            for (HemfRecord record : ex) {
                ...
                if (record.getEmfRecordType() == HemfRecordType.comment) {
                    handleCommentData(((HemfComment.EmfComment) record).getCommentData(),
                                    parseState, xhtml, context);
                } else if (record.getEmfRecordType().equals(HemfRecordType.extTextOutW)) {
                    handleExtTextOut((HemfText.EmfExtTextOutW) record, parseState, buffer, xhtml,
                                    fudgeFactorX, StandardCharsets.UTF_16LE);
                } else if (record.getEmfRecordType().equals(HemfRecordType.extTextOutA)) {
                    // do something better than assigning utf8.
                    handleExtTextOut((HemfText.EmfExtTextOutA) record, parseState, buffer, xhtml,
                                    fudgeFactorX, StandardCharsets.UTF_8);
                }
    ...
    private void handleExtTextOut(HemfText.EmfExtTextOutA record, ParseState parseState,
        ...
        if (parseState.lastRectangle.getY() > -1 && deltaGreaterThan(
                        parseState.lastRectangle.getMinY(), currRectangle.getMinY(), 0.0001)) {
            xhtml.startElement("p");
            xhtml.characters(buffer.toString());
            xhtml.endElement("p");
            buffer.setLength(0);
        } else if (parseState.lastRectangle.getX() > -1 && deltaGreaterThan(currRectangle.getMinX(),
    ...
    private Rectangle2D getCurrentRectangle(HemfText.EmfExtTextOutA extTextOutA) {
        ...
        // via TIKA-4432, if the rectangle is 0,0,0,0 then back-off to the bounds ignored, if those
        // exist
    ...
    private void handleCommentData(HemfComment.EmfCommentData commentData, ParseState parseState,
        ...
        if (commentData instanceof HemfComment.EmfCommentDataMultiformats) {
            if (parseState.extractor == null) {
                parseState.extractor = EmbeddedDocumentUtil.getEmbeddedDocumentExtractor(context);
            }
            handleMultiFormats((HemfComment.EmfCommentDataMultiformats) commentData, xhtml,
            ...
            String val = tryToReadAsString(
                            (((HemfComment.EmfCommentDataGeneric) commentData).getPrivateData()));
        ...
        if (embeddedDocumentExtractor.shouldParseEmbedded(embeddedMetadata)) {
            try (TikaInputStream tis = TikaInputStream.get(bytes)) {
                embeddedDocumentExtractor.parseEmbedded(tis,
                                new EmbeddedContentHandler(contentHandler), embeddedMetadata, true);
    ...
    private void handleMultiFormats(HemfComment.EmfCommentDataMultiformats commentData,
                    ContentHandler handler, EmbeddedDocumentExtractor embeddedDocumentExtractor)
                    throws IOException, TikaException, SAXException {
