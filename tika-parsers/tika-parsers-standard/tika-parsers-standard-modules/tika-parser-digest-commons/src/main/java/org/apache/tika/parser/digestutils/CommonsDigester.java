...
/**
 * Implementation of {@link org.apache.tika.parser.DigestingParser.Digester} that relies on
 * commons.codec.digest.DigestUtils to calculate digest hashes.
 * <p>
 * This digester tries to use the regular mark/reset protocol on the InputStream. However, this
 * wraps an internal BoundedInputStream, and if the InputStream is not fully read, then this will
 * reset the stream and spool the InputStream to disk (via TikaInputStream) and then digest the
 * file.
 */
public class CommonsDigester extends CompositeDigester {

    /**
     * Include a string representing the comma-separated algorithms to run: e.g. "md5,sha1". If you
     * want base 32 encoding instead of hexadecimal, add ":32" to the algorithm, e.g. "md5,sha1:32".
     * If you want uppercase digests for the hexadecimal encoder, use uppercase in the algorithm
     * name, e.g. "MD5".
     * <p/>
    ...
    /**
     * @param markLimit limit for mark/reset; after this limit is hit, the stream is reset and
     *        spooled to disk
    ...
    private static DigestingParser.Digester[] buildDigesters(int markLimit,
        ...
        for (DigestAlgorithm algorithm : algorithms) {
            digesters[i++] = new InputStreamDigester(markLimit, algorithm.getJavaName(),
                            algorithm.name(), new HexEncoder(false));
        }
        return digesters;
    }

    /**
     * This returns digest algorithms only. It does not understand the encoding syntax, e.g.
     * "MD5:32" (base 32 encoding of MD5). To parse those, see
     * {@link #CommonsDigester(int, String)}.
     *
