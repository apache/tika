...
/**
 * Classes that implement this must be able to detect on a ZipFile and in streaming mode. In
 * streaming mode, each ziparchiventry is "updated" and then
 * {@link #streamingDetectFinal(StreamingDetectContext)} is called for a final decision.
 * <p>
...
public interface ZipContainerDetector extends Serializable {
    ...
    /**
     * If detection is successful, the ZipDetector should set the zip file or OPCPackage in
     * TikaInputStream.setOpenContainer()
     *
    ...
    /**
     * Try to detect on a specific entry. Detectors are allowed to store state (e.g. "remember what
     * they've seen") in the {@link StreamingDetectContext}
     *
     * @param zae
     * @return
     */
