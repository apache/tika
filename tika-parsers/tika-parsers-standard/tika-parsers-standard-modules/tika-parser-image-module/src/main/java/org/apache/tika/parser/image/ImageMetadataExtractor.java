...
/**
 * Uses the <a href="http://www.drewnoakes.com/code/exif/">Metadata Extractor</a> library to read
 * EXIF and IPTC image metadata and map to Tika fields.
 * <p/>
...
public class ImageMetadataExtractor {
    ...
    public static final String UNKNOWN_IMG_NS =
                    "img" + TikaCoreProperties.NAMESPACE_PREFIX_DELIMITER;
     ...
     * @param handlers handlers in order, note that handlers may override values from earlier
     *        handlers
     */
    ...
    public void parseRawXMP(byte[] xmpData) throws IOException, SAXException, TikaException {
        ...
        try (InputStream decoded =
                        UnsynchronizedByteArrayInputStream.builder().setByteArray(xmpData).get()) {
     ...
     */
    protected void handle(com.drew.metadata.Metadata metadataExtractor) throws MetadataException {
    ...
    /**
     * Mimics the behavior from TIKA-314 of copying all extracted tags to tika metadata using field
     * names from Metadata Extractor.
     */
    ...
    /**
     * Copies all fields regardless of directory, if the tag name is not identical to a known
     * Metadata field name. This leads to more predictable behavior than
     * {@link CopyAllFieldsHandler}.
     */
    ...
    static class ExifHandler implements DirectoryHandler {
        ...
        /**
         * EXIF may contain image description, although with undefined encoding. Use IPTC for other
         * annotation fields, and XMP for unicode support.
         */
        ...
        public void handlePhotoTags(Directory directory, Metadata metadata) {
            ...
            if (directory.containsTag(ExifSubIFDDirectory.TAG_EXIF_IMAGE_WIDTH)) {
                ...
                // check for null because this could overwrite earlier set width if the value is
                // null
                if (width != null) {
        ...
        public void handleDateTags(Directory directory, Metadata metadata)
            ...
            if (directory.containsTag(ExifSubIFDDirectory.TAG_DATETIME_ORIGINAL)) {
                ...
                if (original != null) {
                    String datetimeNoTimeZone = dateUnspecifiedTz.format(original); // Same time
                                                                                    // zone as
                                                                                    // Metadata
                                                                                    // Extractor
                                                                                    // uses
    ...
    /**
     * Reads image comments, originally TIKA-472. Metadata Extractor does not read XMP so we need to
     * use the values from Iptc or EXIF
     */
        ...
        public void handle(Directory directory, Metadata metadata) throws MetadataException {
            ...
            if (directory.containsTag(IptcDirectory.TAG_CAPTION)) {
                metadata.set(TikaCoreProperties.DESCRIPTION,
                                ...
                                // but the exiv2 command does not so we change to line feed here
                                // because that is less surprising to users
