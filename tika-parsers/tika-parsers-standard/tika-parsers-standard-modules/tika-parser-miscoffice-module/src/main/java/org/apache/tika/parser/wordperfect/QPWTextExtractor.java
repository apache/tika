...
/**
 * Extracts text from a Quattro Pro document according to QPW v9 File Format. This format appears to
 * be compatible with more recent versions too.
 *
    ...
    public void extract(InputStream input, XHTMLContentHandler xhtml, Metadata metadata)
        ...
        if (rootNode == null || !rootNode.hasEntry(OLE_DOCUMENT_NAME)) {
            throw new UnsupportedFormatException("Unsupported QuattroPro file format. "
                            + "Looking for OLE entry \"" + OLE_DOCUMENT_NAME + "\". Found: "
        ...
        try (WPInputStream in =
                        new WPInputStream(pfs.createDocumentInputStream(OLE_DOCUMENT_NAME))) {
