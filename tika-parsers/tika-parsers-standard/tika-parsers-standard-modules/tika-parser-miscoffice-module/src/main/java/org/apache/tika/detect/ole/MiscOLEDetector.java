...
public class MiscOLEDetector implements Detector {
     ...
     * streams within the file.
     *
     * @deprecated Use {@link #detect(Set, DirectoryEntry)} and pass the root entry of the
     *             filesystem whose type is to be detected, as a second argument.
     */
    @Deprecated
        ...
        if (names == null) {
            ...
            try {
                if (input.read() != 0xd0 || input.read() != 0xcf || input.read() != 0x11
                                ...
                                || input.read() != 0xb1 || input.read() != 0x1a
                                || input.read() != 0xe1) {
                    return MediaType.OCTET_STREAM;
                }
            } catch (IOException e) {
