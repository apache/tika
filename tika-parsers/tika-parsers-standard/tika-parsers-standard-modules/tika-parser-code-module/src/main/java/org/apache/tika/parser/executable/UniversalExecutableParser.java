...
public class UniversalExecutableParser implements Parser {
    ...
    public void parseMachO(XHTMLContentHandler xhtml, EmbeddedDocumentExtractor extractor,
                    Metadata metadata, InputStream stream, byte[] first4)
        ...
        int archStructSize = 4 /* cputype */ + 4 /* cpusubtype */
                        + (is64 ? 8 /* offset */ + 8 /* size */ + 4 /* align */ + 4 /* reserved */
        ...
        if (archsCount > MAX_ARCHS_COUNT) {
            throw new TikaException("Number of architectures=" + archsCount
                            + " greater than max allowed=" + MAX_ARCHS_COUNT);
        }
        ...
        for (int archIndex = 0; archIndex < archsCount; archIndex++) {
            ...
            if (!unsortedOffsets && archIndex > 0
                            && offset < (long) offsetAndSizePerArch[archIndex - 1].getLeft()) {
                unsortedOffsets = true;
            }
            long size = is64 ? (isLE ? EndianUtils.readLongLE(stream)
                            : EndianUtils.readLongBE(stream))
                            : (isLE ? EndianUtils.readIntLE(stream)
                                            : EndianUtils.readIntBE(stream));
            ...
            if (size < 0 || size > MAX_ARCH_SIZE) {
                throw new TikaException(
                                "Arch size=" + size + " must be > 0 and < " + MAX_ARCH_SIZE);
            }
        ...
        if (unsortedOffsets) {
            Arrays.sort(offsetAndSizePerArch,
                            Comparator.comparingLong(entry -> (long) entry.getLeft()));
        }
