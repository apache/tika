...
/**
 * <p>
 * This was added in Tika 1.24 as an alpha version of a text extractor that builds the text from the
 * marked text tree and includes/normalizes some of the structural tags.
 * </p>
 *
 * @since 1.24
 */
...
public class PDFMarkedContent2XHTML extends PDF2XHTML {
    ...
    public static void process(PDDocument pdDocument, ContentHandler handler, ParseContext context,
        ...
        try {
            pdfMarkedContent2XHTML = new PDFMarkedContent2XHTML(pdDocument, handler, context,
                            metadata, config);
        } catch (IOException e) {
    ...
    protected void processPages(PDPageTree pageTree) throws IOException {
        ...
        if (pageRefs.size() != pdDocument.getNumberOfPages()) {
            throw new IOException(new TikaException("Couldn't find the right number of page refs ("
                            + pageRefs.size() + ") for pages (" + pdDocument.getNumberOfPages()
                            + ")"));
        }
    ...
    private Map<MCID, String> loadTextByMCID(PDPageTree pageTree, List<ObjectRef> pageRefs)
                    throws IOException {
        int pageCount = 1;
        ...
        for (PDPage page : pageTree) {
            ...
            for (PDMarkedContent c : ex.getMarkedContents()) {
                ...
                for (Object o : objects) {
                    ...
                    /*
                     * TODO: do we want to do anything with these? TODO: Are there other types of
                     * objects we need to handle here? else if (o instanceof PDImageXObject) {
                     * 
                     ...
                     * 
                     * } else { throw new RuntimeException("can't handle "+o.getClass()); }
                     */
                }
    ...
    /**
     * In PDF land, MCID are integers that should be unique _per page_. This class includes the
     * object ref to the page and the mcid so that this should be a cross-document unique key to
     * given content.
     * <p>
     * If the mcid integer == -1, that means that there is text on the page not assigned to any
     * marked content.
     */
