...
public class MatroskaDetectorTest {
    ...
    public void testDetectMKV() throws IOException {
        assertEquals(MediaType.application("x-matroska"), detector.detect(
                        getResourceAsStream("/test-documents/sample-mkv.noext"), new Metadata()));

        assertEquals(MediaType.application("x-matroska"), detector.detect(
                        getResourceAsStream("/test-documents/testMKV.mkv"), new Metadata()));
    ...
    public void testDetectWEBM() throws IOException {
        assertEquals(MediaType.video("webm"), detector.detect(
                        getResourceAsStream("/test-documents/sample-webm.noext"), new Metadata()));
    }
