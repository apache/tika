...
/**
 * ****************************************************************************** Copyright (C)
 * 2005-2016, International Business Machines Corporation and * others. All Rights Reserved. *
 * ******************************************************************************
 */
...
/**
 * <code>CharsetDetector</code> provides a facility for detecting the charset or encoding of
 * character data in an unknown format. The input data can either be from an input stream or an
 * array of bytes. The result of the detection operation is a list of possibly matching charsets,
 * or, for simple use, you can just ask for a Java Reader that will will work over the input data.
 * <p>
 * Character set detection is at best an imprecise operation. The detection process will attempt to
 * identify the charset that best matches the characteristics of the byte data, but the process is
 * partly statistical in nature, and the results can not be guaranteed to always be correct.
 * <p>
 * For best accuracy in charset detection, the input data should be primarily in a single language,
 * and a minimum of a few hundred bytes worth of plain text in the language are needed. The
 * detection process will attempt to ignore html or xml style markup that could otherwise obscure
 * the content.
 * <p>
 *
 * @stable ICU 3.4
...
public class CharsetDetector {
    ...
    static {
        ...
        list.add(new CSRecognizerInfo(
                        new CharsetRecog_mbcs.CharsetRecog_euc.CharsetRecog_gb_18030(), true));
    ...
    /*
     * The following items are accessed by individual CharsetRecongizers during the recognition
     * process
     *
     */
     ...
     * <p>
     * <b>Note:</b> Multiple different charset encodings in a same family may use a single shared
     * name in this implementation. For example, this method returns an array including "ISO-8859-1"
     * (ISO Latin 1), but not including "windows-1252" (Windows Latin 1). However, actual detection
     * result could be "windows-1252" when the input data matches Latin 1 code points with any
     * points only available in "windows-1252".
     *
     * @return an array of the names of all charsets supported by <code>CharsetDetector</code>
     *         class.
     * @stable ICU 3.4
     */
    ...
    /**
     * Set the declared encoding for charset detection. The declared encoding of an input text is an
     * encoding obtained from an http header or xml declaration or similar source that can be
     * provided as additional information to the charset detector. A match between a declared
     * encoding and a possible detected encoding will raise the quality of that detected encoding by
     * a small delta, and will also appear as a "reason" for the match.
     * <p>
     * A declared encoding that is incompatible with the input data being analyzed will not be added
     ...
     * Set the input text (byte) data whose charset is to be detected.
     * <p>
     * The input stream that supplies the character data must have markSupported() == true; the
     * charset detection process will read a small amount of data, then return the stream to its
     * original position via the InputStream.reset() operation. The exact amount that will be read
     * depends on the characteristics of the data itself.
     *
     * @param in the input text of unknown encoding
     * @return This CharsetDetector
     ...
     * <p>
     * Note though, that because the detection only looks at the start of the input data, there is a
     * possibility that the returned charset will fail to handle the full set of input data.
     *
     * @return a CharsetMatch object representing the best matching charset, or <code>null</code> if
    ...
    /**
     * Return an array of all charsets that appear to be plausible matches with the input data. The
     * array is ordered with the best quality match first.
     *
    ...
    /**
     * Autodetect the charset of an inputStream, and return a Java Reader to access the converted
     * input data.
     * <p>
     * This is a convenience method that is equivalent to
     * <code>this.setDeclaredEncoding(declaredEncoding).setText(in).detect().getReader();</code>
     * <p>
     * For the input stream that supplies the character data, markSupported() must be true; the
     * charset detection will read a small amount of data, then return the stream to its original
     * position via the InputStream.reset() operation. The exact amount that will be read depends on
     * the characteristics of the data itself.
     *
     * @param in The source of the byte data in the unknown charset.
     * @param declaredEncoding A declared encoding for the data, if available, or null or an empty
    ...
    /**
     * Autodetect the charset of an inputStream, and return a String containing the converted input
     * data.
     * <p>
     * This is a convenience method that is equivalent to
    ...
    /**
     * Enable filtering of input text. If filtering is enabled, text within angle brackets ("&lt;"
     * and "&gt;") will be removed before detection.
     *
    ...
    /*
     * MungeInput - after getting a set of raw input data to be analyzed, preprocess it by removing
     * what appears to be html markup.
     */
    ...
    /**
     * Enable or disable individual charset encoding. A name of charset encoding must be included in
     * the names returned by {@link #getAllDetectableCharsets()}.
     *
     * @param encoding the name of charset encoding.
     * @param enabled <code>true</code> to enable, or <code>false</code> to disable the charset
     *        encoding.
     * @return A reference to this <code>CharsetDetector</code>.
     * @throws IllegalArgumentException when the name of charset encoding is not supported.
     * @internal
     * @deprecated This API is ICU internal only.
     */
    @Deprecated
