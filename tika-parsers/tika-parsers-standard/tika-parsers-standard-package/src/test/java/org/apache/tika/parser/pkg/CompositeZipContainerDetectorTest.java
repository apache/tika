...
public class CompositeZipContainerDetectorTest extends TikaTest {
     ...
     * TODO these tests!
     * 
     * @Test public void testODT() throws Exception { try (InputStream input =
     * ODFParserTest.class.getResourceAsStream( "/test-documents/testODFwithOOo3.odt")) { Metadata
     * metadata = new Metadata(); MediaType mt = zipContainerDetector.detect(input, metadata);
     * assertEquals(ODT_TEXT, mt); } }
     * 
     * @Test public void testIWorks() throws Exception { //have to have marklimit in
     * ZipContainerDetector > 100000 for this to work try (InputStream input =
     * ODFParserTest.class.getResourceAsStream( "/test-documents/testPages.pages")) { Metadata
     * metadata = new Metadata(); MediaType mt = zipContainerDetector.detect(input, metadata);
     ...
     * assertNotNull(is); TikaConfig tikaConfig = new TikaConfig(is); try (InputStream input =
     * ODFParserTest.class.getResourceAsStream( "/test-documents/testPages.pages")) { Metadata
     * metadata = new Metadata(); MediaType mt = tikaConfig.getDetector().detect(input, metadata);
     * assertEquals("application/zip", mt.toString()); } }
     * 
     * @Test public void testXPS() throws Exception { for (String file : new
     * String[]{"testXPS_various.xps", "testPPT.xps"}) { long start = System.currentTimeMillis();
     * try (InputStream input = ODFParserTest.class.getResourceAsStream( "/test-documents/" + file))
     * { MediaType mediaType = streamingZipDetector.detect(input, new Metadata());
     * assertEquals(ZipContainerDetectorBase.XPS, mediaType); } try (TikaInputStream input =
     * TikaInputStream.get( Paths.get(ODFParserTest.class.getResource( "/test-documents/" +
     * file).toURI()))) { MediaType mediaType = zipContainerDetector.detect(input, new Metadata());
