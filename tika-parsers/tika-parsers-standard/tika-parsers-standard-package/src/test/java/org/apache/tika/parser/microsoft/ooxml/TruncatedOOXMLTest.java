...
public class TruncatedOOXMLTest extends TikaTest {
    ...
    public void testTruncatedStreamDetection() throws Exception {
        ...
        try (InputStream is = new BufferedInputStream(TruncatedOOXMLTest.class
                        .getResourceAsStream("/test-documents/testWORD_truncated.docx"))) {
            MediaType mediaType = detector.detect(is, metadata);
            assertEquals(MediaType.application(
    ...
    public void testTruncatedPathDetection() throws Exception {
        Detector detector = DEFAULT_TIKA_CONFIG.getDetector();
        Metadata metadata = new Metadata();
        try (TikaInputStream tis = TikaInputStream.get(TruncatedOOXMLTest.class
                        .getResourceAsStream("/test-documents/testWORD_truncated.docx"))) {
            tis.getPath();
