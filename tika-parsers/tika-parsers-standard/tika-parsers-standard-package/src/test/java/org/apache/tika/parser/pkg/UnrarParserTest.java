...
public class UnrarParserTest extends AbstractPkgTest {
    ...
    public void testEmbedded() throws Exception {
        ...
        String[] expectedResources = {"testHTML.html", "testEXCEL.xls", "testOpenOffice2.odt",
                        "testPDF.pdf", "testPPT.ppt", "testRTF.rtf", "testTXT.txt", "testWORD.doc",
                        "testXML.xml"};
        ...
        for (String resource : expectedResources) {
            assertEquals(1, metadataList.stream()
                            .filter(m -> m.get(TikaCoreProperties.ORIGINAL_RESOURCE_NAME) != null
                                            && m.get(TikaCoreProperties.ORIGINAL_RESOURCE_NAME)
                                                            .contains(resource))
                            .count());
        }
    }
