...
* `tika-server/docker-build/{minimal,full}/Dockerfile.snapshot` — snapshot builds; keep the
  package lists in step with the release Dockerfiles (`docker-tool.sh test-uat-snapshot` runs
  the UAT against images built from these, ffmpeg asserted on full)
* `tika-grpc/docker-build/Dockerfile` — `apache/tika-grpc` release *and* snapshot builds
* `.github/workflows/docker-release.yml` — the release publishing workflow
* `.github/workflows/docker-snapshot.yml` — the snapshot publishing workflow (auto on push to `main`, skipped when the version is not a `-SNAPSHOT`)
...
full::
Adds Tesseract OCR, GDAL, ImageMagick, ffmpeg, and Microsoft fonts.
...
* The Tika release vote has passed and the artifacts have been moved from
  `dist/dev` to `dist/release`, so `tika-server-standard-<TAG>.zip` and its
  `.asc` are live on `dlcdn.apache.org`/`downloads.apache.org`. The `apache/tika`
  Dockerfiles download and GPG-verify that zip at build time. (The
  `apache/tika-grpc` job builds from source instead and needs nothing from dist.)
* The release tag (e.g. `4.0.0-alpha-1`) exists in the repo. `release:prepare`
  creates the `X.Y.Z-rcN` tag; the GA `X.Y.Z` tag is pushed by hand after the
  vote passes.
...
Each workflow run publishes up to three tags per image, all pointing at the same
manifest digest:
...
A successful run takes ~30–45 minutes (multi-arch builds are slow under qemu
emulation, especially the full image). The `apache/tika` images build for
`linux/amd64`, `linux/arm64`, and `linux/s390x`; `apache/tika-grpc` builds for
...
alpha/beta/RC builds — any hyphen in the tag suppresses it, so those tags stay
on 3.x. No workflow edit is needed at GA: the bare `X.Y.Z` tag has no hyphen, so
the 4.0.0 GA run moves `apache/tika:latest` and `apache/tika:latest-full` on its
own.
...
Any 3.x patch release (e.g. a 3.3.0.1 with a CVE fix) is published from the legacy https://github.com/apache/tika-docker[apache/tika-docker]
