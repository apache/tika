...
* *Docker Hub* — `apache/tika-grpc`, the ready-to-run server image. Built and
  published by `.github/workflows/docker-release.yml`, which assembles its own
  build context (grpc jar + runtime deps + every pipes-plugin zip +
  parser-package jars) after a plain `mvn install`.
