...
|Container deployers. `apache/tika` bundles the standard parsers and the
file-system pipes plugin; `apache/tika-grpc` additionally bundles every pipes
plugin and parser package.
|===
...
|— (mount the shaded jar into `/tika-extras/`)
|inside the image

|`tika-pipes-<plugin>` (each — solr, http, s3, kafka, …)
|✓ slim jar
|✓ pf4j zip distribution
|only `tika-pipes-file-system`
...
|`tika-grpc-<v>.zip` (runnable layout, built only under `-Pdocker`)
...
* xref:maintainers/site.adoc[Publishing the Documentation Site]
