...
downloads and GPG-verifies them, so promote dist in the substep above and let it
propagate — `workflow_dispatch` skips the auto-wait that the tag-push path has):
+
[source,bash]
----
...
natives), so it runs ~600+ MB, much of it duplicating dependencies already under
`lib/`. The bloat lands in the Docker image, not in a release artifact; reducing
