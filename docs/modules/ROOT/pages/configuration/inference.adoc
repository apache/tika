...
recognition and document-level tasks. The `engines` map itself, and the
...
`requestParameters`, `imageInput`, `mediaInput` (see <<media>>).
...
----
{
  ...
  "parsers": [
    { "default-parser": {} },
    { "pdf-parser": { "inference": { "input": ["TEXT", "PAGES"] },
...
[source,json]
----
{ "parse-context": { "media": { "segment": { "seconds": 30, "overlap": 5 }, "maxSegments": 480 } } }
----

Those are the defaults: 30 s windows, each starting 25 s after the last, so any event of
5 s or less is whole in at least one segment; `maxSegments` caps a document, the rest of the
...
----
[
  { "producer": "jina-video", "modality": "visual", "correlator": "t:25000-55000",
    "vector": "<base64 float32>", "locators": { "temporal": [{"start_ms": 25000, "end_ms": 55000}] } },
  { "producer": "jina-audio", "modality": "audio", "correlator": "t:25000-55000",
    "vector": "<base64 float32>", "locators": { "temporal": [{"start_ms": 25000, "end_ms": 55000}] } }
]
----
...
Jina's documented limits: images 5 MB, PDFs 8 MB and one per request; audio as WAV, MP3,
FLAC, OGG, M4A or Opus with no documented duration cap; video as 32 uniformly sampled frames
...
as one input is 32 frames for the film, a 30 s segment is about one frame a second. Byte
