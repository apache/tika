...
PDFs natively with the model's vision capabilities once the default `pdf-parser` is excluded (see
below); an enricher never displaces the parser for a type.
...
Named under `parsers`, the VLM is also the content enricher for the image types it advertises, so
embedded images in every other document type are sent to it too. Add a `content-enrichers` list to
choose a different engine for those, or `"content-enrichers": []` for none; see
xref:configuration/index.adoc[Configuration].
