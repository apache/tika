...
at startup names the collision and the winner; to pin the engine deterministically, name it in
the top-level `text-recognizers` list (`tess4j-parser` or `tesseract-ocr-parser`; see
...
advertises no media types: named under `text-recognizers` that fails
config load with a message naming it; under `parsers` (the deprecated
4.0 shape) it is logged at startup and the parser does nothing.
