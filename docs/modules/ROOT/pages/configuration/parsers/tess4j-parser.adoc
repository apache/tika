...
the top-level `content-enrichers` list (`tess4j-parser` or `tesseract-ocr-parser`; see
...
advertises no media types: named under `content-enrichers` that fails
config load with a message naming it; under `parsers` it is logged at
startup and the parser does nothing.
