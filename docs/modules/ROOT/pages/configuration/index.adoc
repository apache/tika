...
config file, so a reference resolves there too. Per-request configuration (`/config` endpoints,
presets, pipes tuples) is never interpolated: a request cannot read the server's environment.
...
* xref:configuration/pages.adoc[Pages] — rendering, OCR policy and page images, for every parser that renders
* xref:configuration/parsers/pdf-parser.adoc[PDFParser] — PDF parsing options
* xref:configuration/parsers/image-parsers.adoc[Image Parsers] — ICC metadata options for JPEG,
  TIFF, HEIF, BPG and WebP
