...
rather than going silently inert. Failures are best-effort: one enricher failing does not stop the others, and every
failure is still reported through the parser's normal exception handling (timeouts abort the
chain immediately). With no `content-enrichers` configured, behavior is unchanged — whichever
OCR engine is on the classpath is used, exactly as before, and a WARN is logged at startup when
several engines collide. Engine names: `tesseract-ocr-parser`, `tess4j-parser`, `openai-vlm-parser`,
