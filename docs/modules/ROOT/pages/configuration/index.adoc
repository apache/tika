...
those the last registered wins (user-supplied classes register after Tika's). A WARN names the
collision and the winner. Engine names: `tesseract-ocr-parser`, `tess4j-parser`, `openai-vlm-parser`,
...
advertises the real types it enriches. A composite never lets an enricher displace the parser for a
type, so `tesseract-ocr-parser` advertising `image/png` does not take PNGs away from
`image-parser`. The `image/ocr-*` pseudo-types that routed OCR before 4.1 are retired: a
third-party engine still advertising them is treated as a text recognizer for the real types,
with a WARN, until 5.0, and nothing else in Tika produces or accepts them any more.
