...
real type, so naming them here just works, and an entry's `_mime-include`/`_mime-exclude` name
real types too: `image/tiff`, not `image/ocr-tiff`.) *Every* enricher matching a media type runs, in the
