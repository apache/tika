...
Content enrichers are parsers that a container parser *invokes* on bytes it has already parsed:
an OCR engine run on an embedded image or a rendered PDF page, a VLM that transcribes or tags it.
Two recipes cover almost every deployment.

*Choose the engine.* Name it here; every default parser stays loaded, and `image-parser` and
`pdf-parser` call it:
...
*Turn enrichment off.* An empty list is authoritative:
...
Engine names: `tesseract-ocr-parser`, `tess4j-parser`, `openai-vlm-parser`, `claude-vlm-parser`,
...
`image/ocr-*` spellings are refused at load. *Every* enricher matching a media type runs, in the
order listed, so an OCR engine followed by a VLM that tags images is two entries, both invoked
per image. Two text recognizers on one type both run and their text is concatenated; a WARN at
startup names them, since that is almost never intended. The list is authoritative: a media type no configured enricher matches gets no
enrichment, never a classpath engine you did not name. A named engine that reports no media
...
list cannot stand in for an unreachable engine. Failures are best-effort: one enricher failing
...
When the key is absent, the loader resolves one engine per media type from the enrichers among
...
engine silently, by design. Name the engine in `content-enrichers` to stop depending on any of
...
another parser it is never dispatched to: startup logs that it then acts only as the content
enricher, and warns if it never runs at all. Prefer `content-enrichers` for the engine and keep
`parsers` for parsers.

=== Text recognizers

An enricher that produces the document's *text*, such as an OCR engine or a VLM prompted to
transcribe, implements `org.apache.tika.parser.enricher.TextRecognizer`. Only text recognizers
count when a caller asks whether an engine can stand in for text it already has: the PDF parser's
`AUTO` OCR strategy replaces a page's extracted text only if the enricher recognizes text and
actually wrote some, and keeps the extracted text otherwise. Annotating enrichers such as an
image-embedding parser still run on the rendered page; they never displace text. The bundled OCR
engines declare the capability; a VLM parser declares it through its `textRecognizer` option
(default `true`), which you set to `false` for a captioning or tagging prompt.

=== For engine authors

Enrichers are found by interface, not by media type: implement
