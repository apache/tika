...
rather than going silently inert; that check is made against the engine itself, so a
`_mime-include` list cannot stand in for an unreachable engine. Failures are best-effort: one enricher failing does not stop
the others, and every failure is still reported through the parser's normal exception handling
(timeouts abort the chain immediately). With no `content-enrichers` configured, the enrichers
found among the loaded parsers are the candidates and exactly one runs per media type: an engine
configured under `parsers` beats one `default-parser` discovered on the classpath, and among
those the last registered wins (user-supplied classes register after Tika's). When several
engines in the winning tier claim a type, a WARN at first use names them and the winner; an
engine under `parsers` shadows a classpath engine silently, by design. An entry's
`_mime-include`/`_mime-exclude` applies here too, and on the `default-parser` entry it applies to
every engine inside it. Engine names: `tesseract-ocr-parser`, `tess4j-parser`, `openai-vlm-parser`,
...
`AUTO` OCR strategy replaces a page's extracted text only if a configured enricher recognizes text
and actually wrote some, and keeps the extracted text otherwise. Annotating enrichers such as an
image-embedding parser still run on the rendered page; they never displace text. The bundled OCR
engines and VLM parsers all declare the capability.

Enrichers are found by interface, not by media type: an enricher implements
`org.apache.tika.parser.enricher.ContentEnricher` (or `TextRecognizer`, which extends it) and
advertises the real types it enriches. An engine the classpath supplied is never dispatched to:
`tesseract-ocr-parser` advertising `image/png` does not take PNGs away from `image-parser`, it is
called by it. Named under `parsers`, an engine is also a parser, but only for the types no other
parser there claims (`"parsers": [{"tesseract-ocr-parser": {}}]` alone sends every image straight
to OCR). The `image/ocr-*` pseudo-types that routed OCR before 4.1 are retired: a third-party
engine still advertising them is treated as a text recognizer for the real types, with a WARN
when it is first found, until 5.0. Nothing else in Tika produces them; a
`tk:content-type-parser-override` naming one matches no parser and falls to the empty fallback.
