...
To add extra components — additional ``EncodingDetector``s, ``Parser``s, or their
dependencies — without repackaging the application, drop their jars in a directory
and point the `tika.extras.dir` system property at it:
...
the working directory is never used. Treat the directory as a trusted code
