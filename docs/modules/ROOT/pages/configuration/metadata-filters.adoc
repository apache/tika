...
|`tika-inference`
|*Deprecated in 4.1.0, removed in 4.2.0.* Chunks `tk:content` and calls an embeddings endpoint,
putting the vectors on `tk:chunks`, one document per request. Configure the endpoint under
`engines` and bind it with a `TEXT` xref:configuration/inference.adoc#text[inference binding]
instead: the whole tree in batched requests, before this chain. A request may not supply these
filters in a `metadata-filters` list (they would post text to a caller-chosen endpoint); the
flat `skipEmbedding` override below still works. See
...
whether from the request or from the `parse-context` block of the server's own config — and then
you are responsible for keeping `tk:content` yourself. The top-level `metadata-filters` chain this
page documents is not consulted in this mode.
