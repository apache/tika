...
|`tika-inference`
|Chunks `tk:content` and calls an embeddings endpoint, putting the vectors on `tk:chunks`, one
document per request. Since 4.1.0 a `TEXT` xref:configuration/inference.adoc#text[inference
binding] does the same for the whole tree in batched requests; it runs before this chain and
is not part of it. See xref:migration-to-4x/chunk-strategies.adoc[Chunk Strategies].
