...
`/rmeta/config/{handlerType}`, `/meta/config`) with the JSON above as the multipart `config`
part; this requires `allowPerRequestConfig=true`.
...
Bodies above xref:pipes/configuration.adoc#payload-limits[`maxInlineBytes`] are spooled to disk,
