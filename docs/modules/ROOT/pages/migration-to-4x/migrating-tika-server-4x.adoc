...
|PUT |`/tika/json/\{handler\}` |- |JSON with specified handler (text, txt, html, xml, body, markdown, md)
...
The 3.x `/tika/config` and `/tika/form/config` endpoints have been removed. Per-request configuration is now supplied as a `config` part in a multipart POST to the `/tika/config*` paths shown in the table above (`/tika/config`, `/tika/config/json`, `/tika/config/{text,html,xml,md}`, `/tika/config/json/{handler}`), and requires `allowPerRequestConfig`.

**Migration:** Use `POST /tika/config` (raw) or `POST /tika/config/json` (JSON) with a `config` part in your multipart request.
