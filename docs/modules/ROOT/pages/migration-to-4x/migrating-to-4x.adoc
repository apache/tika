...
* A `_mime-include`/`_mime-exclude` naming `image/ocr-*` fails config load; the message names the
  real type to use instead.
* Setting `tk:content-type-parser-override` to `image/ocr-*` no longer forces OCR; the image parser
  always invokes the OCR engine, and the PDF parser's `ocr.strategy` controls pages.
...
* A custom engine that still advertises the pseudo-types keeps working as a text recognizer for the
  real types, with a WARN at load, until 5.0. Advertise the real types and implement
