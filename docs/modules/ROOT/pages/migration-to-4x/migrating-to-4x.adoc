...
* Put the engine in the top-level `text-recognizers` list rather than under `parsers`; every
  default parser stays loaded. `"text-recognizers": []` turns enrichment off everywhere. An engine
  left under `parsers` keeps working; startup logs when it acts only as the enricher and warns
  when it never runs at all. Startup also logs one line per effective engine.
...
=== Embedding filters deprecated (4.1.0+)

`openai-embedding-filter` and `jina-embedding-filter` are deprecated and will be removed in
4.2.0. Configure the endpoint once under `engines` and bind it with a `TEXT`
xref:configuration/inference.adoc#text[inference binding]; it embeds the whole document tree in
batched requests. Until then they keep working from the server-side config. A request can no
longer name them in a per-request `metadata-filters` list (4.0.x accepted a caller-supplied
`baseUrl` there); the flat per-request `{"openai-embedding-filter": {"skipEmbedding": true}}`
form is unchanged.
