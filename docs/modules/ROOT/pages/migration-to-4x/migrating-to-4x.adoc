...
and a page the parser does not read (`maxPages`) is not rendered. In Java,
`PDFParserConfig.getOcr()` and `getImageStrategy()` are gone: read `getPages()`, configure
through `pages()`; the deprecated setters stay.
