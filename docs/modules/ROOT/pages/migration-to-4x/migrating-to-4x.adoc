...
`_min-width`/`_min-height` on a recognizer entry, `minWidth`/`minHeight` on a binding; 0 restores
4.0). A per-request `ocr` block now changes only the fields it names (4.0 replaced the whole
