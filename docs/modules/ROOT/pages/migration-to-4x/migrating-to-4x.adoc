...
excluding `pdf-parser` goes with it: the 4.2 shape is the PDF parser's `"text": "OCR"` with the
...
=== PDF text policy: `ocr.strategy` becomes `text` (4.1.0+)

The PDF parser's `"ocr": {"strategy": ...}` is now the parser's own `"text"`, since it decides
where every page's text comes from, not only whether OCR runs: `NO_OCR` is `EXTRACT`, `OCR_ONLY`
is `OCR`, `OCR_AND_TEXT_EXTRACTION` is `EXTRACT_AND_OCR`, `AUTO` is unchanged, and `NONE` is new
(no text from any source; pages are still rendered for annotators and inference). The old
spelling loads as an alias, `text` wins when both are set, and a config dump writes `text`.
The XML converter writes `text`. In Java, `OcrConfig.getStrategy()` is gone: read
`PDFParserConfig.getText()`; `OcrConfig.setStrategy` is deprecated. See
