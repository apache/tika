...
IMPORTANT: Some file formats can trigger excessive memory use, infinite loops, or JVM
crashes in the underlying parsing libraries. For production systems processing untrusted
files, use xref:pipes/index.adoc[Tika Pipes] which runs each parse in a forked JVM with
timeouts and memory limits. Alternatively, xref:using-tika/server/index.adoc[tika-server]
and xref:using-tika/grpc/index.adoc[tika-grpc] provide the same robustness as a service.
