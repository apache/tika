...
|gauge
|
|Sync parse/detect tasks (`/tika`, `/rmeta`, `/meta`, `/unpack`, `/detect`) in flight
right now; `/pipes` and `/async` work is not included.
|===
...
next one), `connection_abandoned` (a request was interrupted mid-flight). Alert on `oom`,
`timeout` and `crash`; the rest are expected.
