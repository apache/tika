...
`/tika/config`, `/tika/config/{text,html,xml,md,json}`, `/rmeta/config`,
`/rmeta/config/{handlerType}` and `/meta/config` accept
...
----
{
  "presets": {
    ...
    "no-ocr": { "pages": { "text": "EXTRACT" } },
    "no-enrichment": { "text-recognizers": { "enabled": false }, "inference": { "enabled": false } }
  }
}
----
...
WARNING: An active preset is a public route: anyone who can reach the server can call it, and
it runs at config trust — it can bind components `/config` cannot and raise timeout limits
above the per-request clamp. Vet a preset as you would `allowPerRequestConfig`, scoped to what
the preset turns on. Tika's catalog presets are kept bounded (page and render caps, no OCR
unless the preset is about OCR, no timeout raises); hold your own to the same bar. Startup
logs one line per active preset naming the components it configures.

==== The thumbnail catalog presets

Tika ships two catalog presets in `tika-pipes-core`. They are not active out of the box: the
operator names them in the server's `tika-config.json`, and a server without that line answers
`404` on their routes.

[source,json]
----
{ "presets": { "thumbnail": true, "render-thumbnail": true } }
----

Both render the same two things the same way — the first page of a top-level PDF and the EMF/WMF
thumbnail of a top-level Office document (that metafile only, not the document's pictures), in
colour, fitted into 256 x 256 pixels (one xref:configuration/pages.adoc[`pages`] block, `emit`
with a box and `maxDepth`) — so an image found through one is the image the other returns,
dimensions included. Attachments are not rendered: the selection rule below would refuse their
renders, so the presets do not make them. Both name the standard parsers, so they need
`tika-parsers-standard-package` on the classpath.

`/unpack/preset/thumbnail` — "just give me the thumbnail". The zip holds at most one raster
image per request and nothing else: the stored thumbnail (Office docProps, audio cover art,
raw camera previews, epub, iWork), the rasterized EMF/WMF thumbnail, or the PDF page; `204`
when the document has none. An image file is its own thumbnail: it has no embedded documents,
so it answers `204` too. Text is read from the first PDF page only, no OCR or inference runs,
and embedded documents are parsed two levels deep at most, so the parse is cheap.

`/rmeta/preset/render-thumbnail` — "give me everything, thumbnail included". A full parse
plus the two renders above, so an index gets the thumbnail's type and dimensions from the same
request as the text. Nothing is selected for the client; it applies the rule below. The preset
is a `pages` block plus the metafile parsers' overlays, and they merge field by field with what
the config's `parsers` block sets for `pdf-parser`, `emf-parser` and `wmf-parser`; the `pages`
entry in the config's `parse-context` block is replaced by the preset's, as with any parse-context
layer, so keep server-wide page settings a preset must not lose under `parsers`.

The selection rule, for a client that reads `/rmeta` and wants the answer `thumbnail` gives:
take the first embedded document with an `image/*` `Content-Type` other than EMF, WMF or SVG
that is a `tk:embedded-resource-type` of `THUMBNAIL` at `tk:embedded-depth` 1, a `THUMBNAIL` at
depth 2 carrying `tk:rendering:rendered-by` (the rendering of a vector thumbnail), or a
`RENDERING` at depth 1 (a PDF page). The thumbnail of a document inside an archive is that
document's, not the archive's. Plain `/rmeta` lists stored thumbnails already; the PDF page and
the EMF/WMF rendering exist only when a request turns rendering on, which is what
`render-thumbnail` does.
...
Output format: name the handler once. A format segment sets it -- `/tika/config/xml`,
`/rmeta/config/xml`, and `/unpack/all/{handlerType}` (or `/unpack/all/config/{handlerType}`
with a config part), which selects how `tk:content` is rendered in the metadata `/all` adds
(`/unpack/all/ignore` for none). Plain `/unpack` takes no segment: the `metadata.json` a
Frictionless package carries there uses the config's handler, or the default. With no
segment, a `content-handler-factory` in the
`config` part does. Doing both is a `400`, whether or not the
two agree -- on `/tika` the segment also fixes the route's `Content-Type`, so silently
preferring one would let the response lie about its own body.
...
curl -X POST http://localhost:9998/tika/config/json \
  -F "file=@document.pdf" \
  -F "config={\"pages\":{\"text\":\"EXTRACT\"}};type=application/json"
----
...
POST with multipart for per-request configuration; the handler goes in the path, not the
`config` part:

[source,bash]
----
curl -X POST http://localhost:9998/rmeta/config/xml \
  -F "file=@document.pdf" \
  -F "config={\"pages\":{\"text\":\"EXTRACT\"}};type=application/json"
----
...
[IMPORTANT]
====
By default exception detail is returned in full. Stack traces and their messages can contain the
spooled file's path, the source filename, and fragments of the document. That detail appears in
`tk:exception:*` metadata on **successful** parses as well as in error bodies — including inside
the `*.metadata.json` entries of an `/unpack` zip.

One config setting governs every one of those channels — the metadata values, every error body
built from an exception (`422`, `500`; a `400` echoes the caller's own request problem, `415` and
`404`/`405` carry no exception text, and the plain-text `413` from the request-size filter is
fixed text), and the `message` of a `/pipes` or `/async` result. That includes the
results the parent fabricates when the fork never reported one (`FAILED_TO_INITIALIZE`,
`PAYLOAD_LIMIT_EXCEEDED`, a `TIMEOUT`/`OOM`/crash the fork died before reporting): those carry
either no message at all or the trace of the parent's own infrastructure exception — never
document content — formatted under the same policy:
...
`MESSAGE_REDACTED` keeps class names and frames but strips every message; `REDACTED` keeps class
names only. A per-request config carrying it is refused with a `400` naming the reason, so callers
cannot turn it back off. See
xref:advanced/setting-limits.adoc#_exception_reporting[Exception reporting]. Set it whenever the
server's callers are less trusted than its operator. Prefer it to a `MetadataFilter` that drops
`tk:exception:*`: a filter supplied in a per-request config *replaces* the operator's rather than
adding to it.
====
