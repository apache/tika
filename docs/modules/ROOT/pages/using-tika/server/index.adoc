...
infrastructure — the request-handling process never loads parser libraries directly.
This provides process isolation (a parser crash or OOM cannot take down the server)
...
NOTE: Other behavior — `enableUnsecureFeatures`, CORS, TLS, timeouts — is configured
in the JSON config file (see <<_configuration,Configuration>>), not via CLI flags.
...
WARNING: Only enable `enableUnsecureFeatures` if you have secured access to Tika
