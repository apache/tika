...
appears as an embedded document with its dimensions. The same switch works on `/unpack` and
`/unpack/all`; `/unpack/thumbnail` applies the defaults and returns the selected thumbnail.
...
204 if there is none. `?renderThumbnails=true` applies the thumbnail defaults, see
