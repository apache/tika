...
----
{
  "presets": {
    ...
    "no-ocr": { "pdf-parser": { "text": "EXTRACT" } },
...
[source,json]
----
{ "presets": { "thumbnails": true, "render-thumbnails": true } }
----

Both render the same two things the same way — the first page of a PDF at 96 dpi in colour
(`rendering`, bounded by its `maxImagePixels`) and the EMF/WMF thumbnail of an Office document
(that metafile only, not the document's pictures) — so an image found through one is the image
the other returns, dimensions included. Both name the standard parsers, so they need
`tika-parsers-standard-package` on the classpath.

`/unpack/preset/thumbnails` — "just give me the thumbnail". The zip holds exactly one raster
image per document and nothing else: the stored thumbnail (Office docProps, audio cover art,
...
`/rmeta/preset/render-thumbnails` — "give me everything, thumbnail included". A full parse
...
names only rendering settings, and they merge with what the config's `parsers` block sets for
`pdf-parser`, `emf-parser` and `wmf-parser`; a same-named entry in the config's `parse-context`
block is replaced by the preset's, as with any parse-context layer, so keep server-wide parser
settings under `parsers`.

The selection rule, for a client that reads `/rmeta` and wants the answer `thumbnails` gives:
...
curl -X POST http://localhost:9998/tika/config/json \
  -F "file=@document.pdf" \
  -F "config={\"pdf-parser\":{\"text\":\"EXTRACT\"}};type=application/json"
----
