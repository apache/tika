...
`/rmeta/config/xml`, and `/unpack/all/{handlerType}`, which selects how `tk:content` is
rendered in the metadata `/all` adds (`/unpack/all/ignore` for none; plain `/unpack` carries
no metadata, so it takes no segment). With no segment, a `content-handler-factory` in the
