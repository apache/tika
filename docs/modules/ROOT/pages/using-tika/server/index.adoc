...
Both render the same two things the same way — the first page of a PDF and the EMF/WMF thumbnail
of an Office document (that metafile only, not the document's pictures), in colour, fitted into
256 x 256 pixels (one xref:configuration/pages.adoc[`pages`] block, `emit` with a box) — so an
image found through one is the image the other returns, dimensions included. Both name the
standard parsers, so they need `tika-parsers-standard-package` on the classpath.
