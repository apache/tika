...
Output format on the `/config` routes: name the handler once. A format segment
(`/tika/config/xml`, `/rmeta/config/xml`) sets it; with no segment, a
`content-handler-factory` in the `config` part does. Doing both is a `400`, whether or not the
