...
|`maxRequestSizeBytes`
|`1 GiB`
|Maximum request body in bytes; larger requests are rejected with `413`. Uploads are spooled to
disk, so this bounds how much one request can write. A negative value disables the limit.
