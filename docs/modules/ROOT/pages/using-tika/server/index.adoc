...
`/rmeta/preset/render-thumbnails` — "give me everything, thumbnail included". A full parse as
the server configures it (text, OCR, inference, every embedded document), plus the two renders
above, so an index gets the thumbnail's type and dimensions from the same request as the text.
Nothing is selected for the client; it applies the rule below.
