...
`/unpack/preset/thumbnails`: the zip holds exactly one raster image per document — the stored
thumbnail (Office docProps, audio cover art, raw camera previews, epub, iWork), the rasterized
EMF/WMF thumbnail of an Office document, or the first page of a PDF — and no OCR runs. Activate
it with `"presets": {"thumbnails": true}`; it names the standard parsers, so it needs
