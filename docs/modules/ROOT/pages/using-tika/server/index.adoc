...
EMF/WMF thumbnail of an Office document, or the first page of a PDF at 96 dpi — and answers
`204` when the document has none. Text is read from the first PDF page only and no OCR or
inference runs, so the parse is cheap. It names the standard parsers, so it needs
