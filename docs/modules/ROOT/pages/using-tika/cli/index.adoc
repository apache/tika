...
|`--task-timeout=<ms>`
|Total per-file budget in milliseconds (default: 3600000 -- 1 hour). Replaces the pre-4.0
`--fork-timeout`, which is now a hard error naming this flag and `--progress-timeout` as
its replacements.

|`--progress-timeout=<ms>`
|Stall detector: maximum time with no progress before the fork is killed as hung, in
...
|`-T` or `--timeoutMillis=<ms>`
|Total task timeout per input file in milliseconds. Sets
`parse-context.timeout-limits.totalTaskTimeoutMillis`, merging into any
`timeout-limits` already in the config.
|===
