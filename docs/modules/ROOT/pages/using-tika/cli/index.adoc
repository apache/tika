...
NOTE: In 4.0.0 the inline short forms `-eX` (encoding), `-pX` (password) and `-c<uri>`
(network client) were removed from standard mode. Use `--encoding=X`, `--password=X` and
`--client=<uri>`. Scripts using the old forms now fail with an unrecognized-option error
