...
[CAUTION]
====
Long flags take the GNU double-dash form. `--input /path` works; `-input /path` does not —
`tika-app` rejects single-dash long names with an `IllegalArgumentException` naming the right
form. Short flags take one dash (`-i`, `-eUTF-8`), except `--Xmx=<size>`, which is a long flag
despite looking like a JVM option (there is no `-X` flag).

Three short flags mean different things in the two modes, so read them in context:

[cols="1,2,2"]
|===
|Flag |Standard mode |Pipes mode
...
|`-e<X>`, `--encoding=<X>`
|Use output encoding `<X>`, e.g. `-eUTF-8`.
|===
...
|`-p<X>`, `--password=<X>`
|Use document password `<X>` (encrypted PDFs, OOXML, etc.). No space after `-p`.
...
* Any of `-i`, `--input`, `-o`, `--output`, `--fileList`, `-z`, `--extract`, `--extract-dir`, `-Z`,
  or `-a`/`--async` is present.
...
|`-i`, `--input=<dir>`
|Input directory (a single file also works).
