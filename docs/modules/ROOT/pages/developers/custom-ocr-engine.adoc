...
NOTE: The Java contract on this page is experimental in 4.1.0. Today an engine is called once
per image and writes into the caller's body as it runs; 4.2 batches recognition per document
and places the text after the walk. The intent is additive: an engine that handles one image
at a time keeps working unchanged, called once per image into a buffer, and an engine whose
service takes several images opts in by declaring a batch size and implementing the list call
that 4.2 adds. The JSON that names your engine will not change.
...
* Naming the engine under `parsers`. Deprecated since 4.1.0 and unsupported in 4.2.0: the entry
  means two things at once (configure the engine, and parse the types nobody else claims), and
  startup has to tell you which happened. `engines` plus `text-recognizers` says what you mean.
