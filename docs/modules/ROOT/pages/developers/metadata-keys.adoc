...
metadata. (A handful of `String` constants remain, e.g. `IPTC`'s four `*_WRONG_CASE` fields and
`TikaCoreProperties.EMBEDDED_RESOURCE_TYPE_KEY`; these are deprecated/internal building blocks that
construct a real `Property`'s name — not independent keys — and are excluded from the registry's key
count for that reason.) That closed/open key space is tracked in a generated, build-gated registry, so
adding a key involves one extra step beyond writing the Java.
