...
Inside an array-valued section (`parsers`, `detectors`, `encoding-detectors`,
`metadata-filters`) an element can be specified as:

* **String**: `"pdf-parser"` - creates instance with defaults
* **Object**: `{"pdf-parser": {"sortByPosition": true}}` - creates configured instance

Object-valued sections such as `parse-context` key each entry by name instead, with
the config object as the value.
...
`SelfConfiguring` is a marker interface with no methods: `resolveAll` skips such
components, leaving their JSON in `ParseContext`, and the component reads it where
it needs it. `Parser` extends `SelfConfiguring`, so every parser is self-configuring.
...
`{"parse-context": {"pages": {"text": "EXTRACT_AND_OCR"}}}`
...
That fails with "Unrecognized parse-context entry 'java.lang.Runtime'. Check for a typo; it does
not match any registered component."
...
* Self-configuring components are never resolved into the context map; read them
  with `ParseContextConfig.getConfig(context, "<name>", ConfigClass.class, default)`
