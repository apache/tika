...
exposes the `/v1/chat/completions` and `/v1/models` endpoints in the OpenAI
format. `OpenAIVLMParser` probes `/v1/models` at startup and stays disabled
for the whole run if that probe fails, so the server must answer it.
...
# Tika's OpenAIVLMParser health-checks this at init; a 404 here disables the parser.
@app.get("/v1/models")
def list_models():
...
def main():
    global model, model_name, processor, device
...
# {"object":"list","data":[{"id":"jinaai/jina-vlm","object":"model"}]}
----

== Configure Tika to use it

Point the `OpenAIVLMParser` at your local server and name it as the text recognizer, so it
transcribes embedded images and rendered PDF pages while every default parser stays loaded:
...
----
{
  ...
  "text-recognizers": [ { "engine": "local-vlm" } ]
}
----

Config load fails if the server is not reachable at startup, so start the server first.
