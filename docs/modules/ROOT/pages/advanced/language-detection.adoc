...
* **TikaLanguageDetector** (`tika-langdetect-charsoup`) — a built-in hash-based
...
== Architecture: TikaLanguageDetector
...
python tika-langdetect/tika-langdetect-charsoup/src/test/python/download_corpus.py \
    ~/datasets/lang-detect \
    --max-per-lang 100000
----

This downloads one corpus per language (preferring 100K-sized corpora) from
`downloads.wortschatz-leipzig.de` and writes them in Leipzig format:
...
Use `--merge-all` to download and merge all available corpora per language
(slower, more data).

=== Step 2: Train the Model

The training pipeline is a Java main class that can be run directly:
...
Arguments: `<corpusDir> <outputFile> [numBuckets] [maxPerLang]`
...
3. **Merge language codes** — consolidate duplicate ISO 639-3 codes that refer to
   the same language (e.g., `azj`→`aze`, `cmn`→`zho`, `ekk`→`est`, `lvs`→`lav`,
   `nor`→`nob`, `pes`→`fas`, `plt`→`mlg`, `zsm`→`msa`)
4. **Filter low-resource languages** — drop languages with fewer than 10,000
   sentences after deduplication
5. **Stratified split** — 80% train / 10% dev / 10% test, stratified by language
6. **Global shuffle** — shuffle the entire training file so languages are
   interleaved (critical for mini-batch training)
7. **Pass 1: Initial training** — AdamW optimizer (mini-batch, 1 thread) followed
   by Hogwild! parallel SGD, with within-epoch early stopping
8. **Filter mislabeled sentences** — use the Pass 1 model to predict each training
   sentence; remove sentences not classified as their labeled language
9. **Pass 2: Retrain** — same optimizer schedule on the filtered training data
10. **INT8 quantization** — convert float32 weights to signed 8-bit integers with
    per-class scaling factors
11. **Evaluate** — report quantized model accuracy on the held-out test set
12. **Export** — write the binary LDM1 model file

Each step prints its wall-clock time so bottlenecks are immediately visible.
...
1. **AdamW** (<<loshchilov2019>>) — decoupled weight decay Adam optimizer for the
   ...
   (batch size 128). This quickly learns good initial weights.
2. **Hogwild! SGD** (<<niu2011>>) — lock-free parallel stochastic gradient descent
   for up to 3 more epochs, with linearly decaying learning rate. Each of the
   12 threads processes its slice of the data and writes gradient updates directly
   to shared weight arrays with no synchronization.
