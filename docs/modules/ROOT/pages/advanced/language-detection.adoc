...
The general model covers 229 language classes trained on Wikipedia dumps as the
...
for 16 languages with insufficient Wikipedia coverage. It uses 16,384 hash buckets
...
(bigrams + trigrams + 4-grams + 5-grams + word unigrams). The richer n-gram
features compensate for the reduced token count at short text lengths.
...
For the default classpath resources (general model ~3.6 MB, short-text model
~4.3 MB), heap loading is used and the performance difference is negligible.
