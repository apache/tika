...
* the per-document JSONL written during the batch run by the
  xref:pipes/plugins/filesystem.adoc#file-system-jsonl-reporter[`file-system-jsonl-reporter`]
  (`--pipesReport` / `-pr` on Profile, `--pipesReportA`/`-pa` and `-pb` on Compare)
...
|`CRASH` |`OOM`/`TIMEOUT` recorded, or `UNSPECIFIED_CRASH` without the shared-mode marker
|`LOST_TO_WORKER_RESTART` |`UNSPECIFIED_CRASH` whose message says `outcome unknown`: a shared-server crash while this request was in flight. Retry once, alone, before blaming the file.
|`NO_PIPES_RECORD` |A report was supplied but has no line for this file
|`NO_PIPES_REPORT_SUPPLIED` |No `--pipesReport` given
|===
...
`eval.*` (tika-eval version, jar sha256, host, args, start/end) and `extracts.*`
(count and a sha256 fingerprint of sorted name+size). If `run-info.json` carries
`run.id`, the pipes report's file name must contain it, or tika-eval refuses to
start -- rerunning a batch into the same extracts dir is otherwise invisible.
`summary.md` opens with the `run_info` tables.
...
absent -- refusing if it holds more than one run-info, since that means the extracts dir was
written by more than one run.
