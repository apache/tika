...
# Copy and extract distribution. The zip has no wrapping directory: the jar,
# lib/ and plugins/ land directly in the current directory.
cp /path/to/tika-app-4.0.0-SNAPSHOT.zip .
unzip tika-app-4.0.0-SNAPSHOT.zip

# Get test files
cp /path/to/tika-main/release-tools/uat/test-files/testPDF.pdf .
cp /path/to/tika-main/release-tools/uat/test-files/test_recursive_embedded.docx .
cp /path/to/tika-main/release-tools/uat/test-files/testHTML.html .
----

Below, `tika-app.jar` stands for the versioned jar the zip actually contains
(`tika-app-4.0.0-SNAPSHOT.jar`). Run every command from the unzipped directory so the
sibling `lib/` and `plugins/` are found.
...
*Expected:* Creates .json files with text content (`tk:content-handler-type` should be `TEXT`).
...
java -jar tika-app.jar --Xmx=2g /tmp/tika-app-test/batch-input /tmp/tika-app-test/heap-out
----
...
=== Issue 2: INFO Message on Every Default-Config Command

Every command that loads the default single-file config prints an INFO message to stderr about
the non-default features TikaCLI turns on. Passing `--config=<file>` skips it; otherwise use
