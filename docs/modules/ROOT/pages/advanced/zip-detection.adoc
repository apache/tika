...
// getBoolean returns null when the property is not set
boolean wasSalvaged = Boolean.TRUE.equals(metadata.getBoolean(Zip.SALVAGED));
----
...
[source,java]
----
// Recommended: detection runs, so salvaging applies
...
[source,java]
----
// Direct parser: no detection, so no salvaging -- may fail on truncated files
...
`zip:detector-zip-file-opened`:: `true` if the detector has a `ZipFile` for the parser to reuse —
either the original or, when `zip:salvaged` is also `true`, the salvaged copy. The `ZipFile`
object is available via `TikaInputStream.getOpenContainer()`.
