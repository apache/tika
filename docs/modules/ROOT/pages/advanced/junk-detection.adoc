...
=== Scoring a string or byte array

[source,java]
----
JunkDetector detector = JunkDetector.loadFromClasspath();

// Score a string directly
JunkScore score = detector.score("The quick brown fox jumps over the lazy dog.");
...
// Score raw UTF-8 bytes (same result; use when you already have bytes)
byte[] utf8 = text.getBytes(StandardCharsets.UTF_8);
JunkScore score2 = detector.score(utf8);
----
...
=== Comparing two charset interpretations
...
JunkDetector.CompareResult result =
    detector.compare(rawBytes, "cp1252", "cp1251");
...
The `delta()` is the absolute difference in z-scores between the two decodings.
As a rough guide:
...
`JunkScore` where `isUnknown()` is true and no z-score is available.
