...
That interaction matters at the defaults: `TesseractOCRConfig.timeoutSeconds`
is 120 seconds, while `progressTimeoutMillis` is 60 seconds. A single
tesseract call taking between 60 and 120 seconds is stopped by the progress
timeout before its own per-process timeout applies. If you OCR large or
high-resolution pages, raise `progressTimeoutMillis` to at least the
per-process OCR timeout.
