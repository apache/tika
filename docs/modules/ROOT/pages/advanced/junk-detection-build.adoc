...
| `cleanVsGarbage`
| Clean English z-score > random high-byte garbage z-score.

| `forwardVsReversedArabic`
| Forward Arabic z-score > codepoint-reversed Arabic z-score.

| `cp1252VsCp1257OnBalticText`
| `compare()` picks cp1257 as the correct encoding for Lithuanian text.
  Delta > 0.1 (weak; Baltic limitation documented above).

| `cp1252VsCp1251OnRussianText`
| `compare()` picks cp1251 as the correct encoding for Russian text.
  Delta > 1.0 (strong; Cyrillic bigrams are highly distinctive).

| `cleanVsShuffledCjk`
| Clean Japanese UTF-8 z-score > byte-shuffled Japanese z-score.
|===
