...
* **Digesting.** If a `DigesterFactory` is configured on the `ParseContext`, it runs against the raw bytes and writes the digest fields (e.g., `tk:digest:SHA256`) to the container metadata before the parse-mode check.
