...
Tika injects `-XX:ActiveProcessorCount=<slice>` into each forked JVM. A fair
share below 2 is clamped to 2 rather than left uncapped: at `slice=1` the
fork's only CPU is fully consumed by parsing, so its socket-reader thread
cannot run, and with no cap at all every fork sizes its GC and JIT pools for
the whole host. The clamp is logged as `slice=2 (clamped; fair share would be
N)`, and with more than one client a `WARN` advises lowering `numClients`.
...
|2  |1 |1 → 2 (clamped) |Tight. Acceptable for low throughput.
|4  |1 |2      |Comfortable single-fork deployment.
|4  |2 |1 → 2 (clamped) |Over-provisioned; consider `numClients=1`.
...
|16 |8 |1 → 2 (clamped) |Doesn't fit 16 cores. Keep at 4 or 6.
|32 |8 |3      |Same shape as 16/4.
|===

The general rule is: pick the largest `numClients` that satisfies
`numClients × 2 + 2 ≤ hostCores`. Beyond that point, adding forks
...
* `slice=2 (clamped; fair share would be N)` — over-provisioned; reduce `numClients`.
...
config section, one set of forks. The auto-sizer has no visibility
...
instance, and `/async` manages its own fork pool directly (not via
...
in the same process is about to start its own `numClients` forks too. The
...
A single fork gets 60. The parent claims the JVM default (about 25% of the container) on top
of whatever the forks take, and metaspace, thread stacks and page cache come out of what is
left; committing the whole container between the two JVMs trades a clean per-document OOM for
an OOM-kill that takes both down. The `pipes-cpu-sizing` summary line reports the decision as
`heap=...`.
...
however many forks that produced. On a host with many cores relative to its RAM, a
...
`numClients × per-fork-heap ≤ 75% of memory`.
====
...
. Measure peak per-fork live-heap with `-Xlog:gc*` (look at the post-GC working set, not the peak before GC).
