...
|`numClients`
|_CPU-derived_
|Number of parallel forked JVM processes. Each processes one document at a time. Defaults to `max(1, min((cores - 2) / 2, 4))` -- roughly half the host's cores, capped at 4. See xref:pipes/cpu-sizing.adoc[Forked-JVM CPU Sizing] for guidance on choosing this value relative to host CPU count.
