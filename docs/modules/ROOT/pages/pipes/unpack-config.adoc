...
To get metadata with no extracted text at all -- one sidecar per file, but no `tk:content` --
select the `IGNORE` handler. On the test corpus below this drops the sidecars from 20,257 to
...
The five knobs interact differently per surface. The defaults are unchanged from 4.0.0.
