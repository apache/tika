...
attached to your `FetchEmitTuple`. The JSON `parse-context` section above is the
...
|`NONE`
|How to determine file extensions for extracted files. See <<_suffix_strategies,Suffix Strategies>>.
...
|DEFAULT, CUSTOM
|`DEFAULT`
|Strategy for generating emit keys. See <<_key_base_strategies,Key Base Strategies>>.
...
|`REGULAR`
|Output format for the ZIP archive. See <<_frictionless_data_package_output,Frictionless Data Package Output>>.
...
`DEFAULT`:: Output key is `\{containerKey}-embed/\{id}\{suffix}`. The `-embed/` separator is fixed; `embeddedIdPrefix` is not consulted.
`CUSTOM`:: Output key is `\{emitKeyBase}\{embeddedIdPrefix}\{id}\{suffix}`.
...
entries written by `includeMetadataInZip`) is Markdown by default. `/unpack` has no format
segment in its path, so on tika-server the only way to change it is a
`content-handler-factory` in the same `config` part. Each entry records the handler it was
written with in `tk:content-handler-type`:

[source,json]
----
{
  "unpack-config": { "outputFormat": "FRICTIONLESS", "includeFullMetadata": true },
