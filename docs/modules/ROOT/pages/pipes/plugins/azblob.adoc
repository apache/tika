...
Reads blobs from an Azure Storage container. It has two modes, chosen by whether
`endpoint`/`container`/`sasToken` are configured:

* **Pinned container** — set all three. The fetch key is the blob name within that container.
* **Per-key SAS URL** — set none of them. The fetch key is the whole blob connection string,
  so each tuple can name a different account and container.

Setting only some of the three fails at startup: they are all-or-nothing.
...
Fetcher, emitter, and iterator wired into a container-to-container pipeline.
