...
|_conditional_
|Required by the matching `credentialsProvider`. `${env:AWS_SECRET_ACCESS_KEY}` keeps the
secret out of the file (xref:configuration/index.adoc#env[Secrets from the environment]).
...
prepends its own `prefix` to that, asking for `incoming/incoming/report.pdf` — every fetch
misses.

Set the prefix on exactly one side: on the iterator (and leave the fetcher's unset), as the
example above does, or on
