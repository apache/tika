...
|`queryTimeoutSeconds`
|`-1`
|Applied via `Statement.setQueryTimeout(int seconds)` only when greater than `0`. `0` and any negative value both leave the driver's own setting alone, so the JDBC contract's explicit "no limit" cannot be requested here.
|===
