...
An array is rejected outright in `fetchers`, `emitters` and `pipes-iterator`. `pipes-reporters` is
the exception: an array there loads zero reporters and reports no error, so keep it an object.
