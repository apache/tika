...
Zips in `plugin-roots` are the only source of plugins. A plugin jar on the application classpath
is not scanned, even though it carries the same `META-INF/extensions.idx`, unless the JVM runs
with `-Dtika.plugins.classpath=true`. That switch exists so `PipesForkParser` can embed without a
plugins directory; it drops the classloader isolation for every plugin found that way, and a
process started with it owns whatever its classpath does.
