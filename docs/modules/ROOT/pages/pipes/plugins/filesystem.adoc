...
* `elapsedMs` — wall-clock time the driver spent on this document: fetch, parse, and any wait for the emit queue; not the emit itself.
...
|`onExists`
|`EXCEPTION`
|What to do when `path` already exists at startup: `EXCEPTION` refuses to start, `APPEND` continues the existing file (terminating a partial last line left by a killed run), `REPLACE` truncates it. There is no `SKIP`, unlike the emitter's `onExists`. The default is deliberately strict — appending a new run onto an old ledger is the mistake this reporter exists to prevent.
