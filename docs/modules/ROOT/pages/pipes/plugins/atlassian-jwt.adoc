...
The fetch key is used as the request URL with no validation, exactly as in the
xref:pipes/plugins/http.adoc#security-notes[HTTP fetcher] — the same server-side request
forgery considerations apply, including the absence of a redirect host allowlist. Treat the
source of fetch keys as fully trusted.
