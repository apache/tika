...
|`idField`
|`id`
|Name of the Solr field that receives the emit key as the document ID (attachments get the emit key plus a random suffix).
...
|`proxyHost` / `proxyPort`
|_none_ / `0`
|Outbound HTTP proxy. `proxyPort` is applied only when positive; the proxy itself is used only when `proxyHost` is set.

|`verifySsl`
|`true`
|Verify server certificates and hostnames. Setting it to `false` accepts any certificate from any host — prefer adding your CA to the JVM truststore.
|===
...
WARNING: The example above is incomplete — `SolrPipesIterator` also validates
`parsingIdField`, `failCountField` and `sizeFieldName` as non-blank, so as shown it fails
at startup with `parsingIdField must not be empty`. Add all three before using it. The
config-example test only checks that the JSON loads, and loading a config never constructs
the iterator, which is why this is not caught.
...
|`filters`
|_empty_
|List of Solr filter queries to scope the iteration (e.g., `["status:NEEDS_REPARSE"]`). The query itself is always `*:*`.

|`idField`
|_required_
|Solr field whose value becomes both the fetch key and the emit key.

|`parsingIdField`
|_required_
|Solr field used as the primary sort key for the cursor-mark paging.

|`failCountField` / `sizeFieldName`
|_required_
|Solr fields added to the query's field list. Validated non-blank even when you have no use for them.

|`additionalFields`
|_empty_
|Further Solr fields to request alongside the four above.
...
|`userName` / `password` / `authScheme` / `proxyHost` / `proxyPort` / `verifySsl`
|Same as the emitter
|See the emitter table above.
|===

[#solr-pipeline]
== Complete Pipeline Example

A filesystem iterator/fetcher feeding the Solr emitter — the usual shape for ingesting a directory into Solr.
