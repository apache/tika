...
|`authScheme`
|`basic`
|Credential scheme. Only `basic` is supported, and only when `userName` is set: the emitter then
fails to build on any other value. With no `userName`, auth is skipped and this setting is ignored
...
|`additionalFields`
|_empty_
|Further Solr fields to request alongside those above. The field list always includes `id` as well, whether or not `idField` is set to it.
