...
* **There is no redirect host allowlist.** Redirects are followed up to `maxRedirects`
  regardless of the target host -- a redirect can send the fetch to any host the server
  can reach, including loopback, link-local, or RFC1918 addresses. Set `maxRedirects` to
  `0` (the default) to follow no redirects at all.
