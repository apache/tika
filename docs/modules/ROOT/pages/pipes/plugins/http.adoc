...
`${env:NAME}` reads one from the environment (xref:configuration/index.adoc#env[Secrets from the environment]).
...
* **TLS certificates are verified by default (`verifySsl: true`).** Not every HTTP-based
  plugin does this — check the plugin page before assuming.
+
